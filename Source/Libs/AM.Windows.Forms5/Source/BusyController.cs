...
/* BusyController.cs -- контроллер состояния занятости
...
/// <summary>
/// Контроллер состояния занятости.
/// </summary>
    ...
    : Component
{
        ...
        )
    {
        ...
        if (!ReferenceEquals (state, null))
        {
            try
            {
                Magna.Logger.LogTrace ("BusyController::Run: " + "before");
            ...
            catch (Exception exception)
            {
                Magna.Logger.LogError
                    (
                        exception,
                        nameof (BusyController) + "::" + nameof (Run)
                    );
                WriteLine
                    (
                        "{0}: {1}",
