...
/// <summary>
/// Контрол, отображающий свойства пера (<see cref="Pen"/>).
/// </summary>
internal sealed class PenInfoControl
    : UserControl
{
    #region Properties
    ...
    {
        Sure.NotNull (penInfo);
        Sure.NotNull (editorService);

        _editorService = editorService;
        InitializeComponent();

        colorPanel!.BackColor = penInfo.Color;

        foreach (var o in Enum.GetValues (typeof (PenAlignment)))
        {
            alignment!.Items.Add (o);
        }

        alignment!.SelectedItem = penInfo.Alignment;

        foreach (var o in Enum.GetValues (typeof (DashStyle)))
        {
            dashStyle!.Items.Add (o);
        }

        dashStyle!.SelectedItem = penInfo.DashStyle;

        foreach (var o in Enum.GetValues (typeof (LineCap)))
        {
            startCap!.Items.Add (o);
            endcap!.Items.Add (o);
        }

        startCap!.SelectedItem = penInfo.StartCap;
        endcap!.SelectedItem = penInfo.EndCap;

        foreach (var o in Enum.GetValues (typeof (LineJoin)))
        {
            lineJoin!.Items.Add (o);
        }

        lineJoin!.SelectedItem = penInfo.LineJoin;

        foreach (var o in Enum.GetValues (typeof (DashCap)))
        {
            dashCap!.Items.Add (o);
        }

        dashCap!.SelectedItem = penInfo.DashCap;
        width!.Value = (decimal)penInfo.Width;
    }
