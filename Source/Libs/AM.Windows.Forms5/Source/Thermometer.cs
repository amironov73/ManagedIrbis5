    ...
    : Control
{
    ...
    /// <summary>
    /// Возникает при изменении текущей температуры.
    /// </summary>
    public event EventHandler? CurrentTemperatureChanged;

    /// <summary>
    /// Возникает при изменении минимальной температуры.
    /// </summary>
    public event EventHandler? MinimalTemperatureChanged;

    /// <summary>
    /// Возникает при изменении максимальной температуры.
    /// </summary>
    ...
    {
        _maximalTemperature = Math.Max
            (
                _maximalTemperature,
                _minimalTemperature
            );
        _currentTemperature = Math.Max
            (
                Math.Min
                    (
                        _currentTemperature,
        ...
        )
    {
        ...
        {
            var globule = new Rectangle
                (
                    0,
                    Height - globuleDiameter,
                    globuleDiameter,
                    globuleDiameter
                );
            ...
            if (innerColumnHeight > 0)
            {
                ...
                var columnGap = new Rectangle
                    (
                        columnPosition + 1,
                        redColumn.Top - columnWidth / 2 + 1,
                        columnWidth - 2,
                        columnWidth - 2
                    );
