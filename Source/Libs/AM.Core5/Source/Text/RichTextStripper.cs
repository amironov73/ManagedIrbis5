...
public static class RichTextStripper
{
    ...
    private static readonly List<string> _destinations = new ()
    {
        "aftncn",
        "aftnsep",
        "aftnsepc",
    ...
    private static readonly Dictionary<string, string> _specialCharacters = new ()
    {
        { "par", "\n" },
        { "sect", "\n\n" },
        { "page", "\n\n" },
        ...
        )
    {
        ...
        if (matches.Count > 0)
        {
            foreach (Match match in matches)
            {
                ...
                var newline = match.Groups[6].Value;
                var tchar = match.Groups[7].Value;
                ...
                {
                    curskip = 0;
                    if (_destinations.Contains (word))
                    {
                        ignorable = true;
                    }
                    else if (ignorable)
                    ...
                    else if (_specialCharacters.ContainsKey (word))
                    {
                        outList.Add
                            (
                                _specialCharacters[word]
                            );
                    }
                    else if (word == "uc")
                    {
        ...
        var result = string.Join (string.Empty, outList.ToArray());
        return result;
    }
