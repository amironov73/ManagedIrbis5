...
/// </summary>
public sealed class PooledMemoryStream
    : Stream, IBufferWriter<byte>
{
    ...
    public PooledMemoryStream
        (
            PooledMemoryStreamManager manager
        )
    {
        _manager = manager;
    }
    ...
    private BufferSegment EnsureCurrentInitialized (int length)
    {
        if (_head is null)
        {
            var current = _head = _current = _manager.Allocate (length);
    ...
    private BufferSegment AllocateAndAppendSegment (BufferSegment current, int length)
    {
        ...
        var next = _manager.Allocate (length);
    ...
    public override void Write (byte[] buffer, int offset, int count)
    {
        ...
        var current = EnsureCurrentInitialized (count);
        ...
        while (count > 0)
        {
            ...
            var segmentOffset = unchecked ((int)(_position - current.RunningIndex));
            var segmentAvailable = current.Length - segmentOffset;
            var writeCount = Math.Min (segmentAvailable, count);
    ...
    public override int Read (byte[] buffer, int offset, int count)
    {
        ...
        var readCount = 0;
        var currentAvailable = (int)(current.RunningIndex + current.Length - _position);
    ...
    public override void SetLength (long value)
    {
        ...
        if (head is null)
        {
            ...
            var currentLength = Math.Min (value32, head.Capacity);
        ...
        var expandSize = (int)(value - head.RunningIndex - head.Length);
        ...
        while (expandSize > 0)
        {
            ...
            var size = Math.Min (array.Length, expandSize);
    ...
    {
        var current = EnsureCurrentInitialized (1);
    ...
    public override async Task CopyToAsync (Stream destination, int bufferSize, CancellationToken cancellationToken)
    {
        ...
        while (current is not null)
        {
            ...
            var offset = (int)(position - current.RunningIndex);
            var count = current.Length - offset;
    ...
    Memory<byte> IBufferWriter<byte>.GetMemory (int sizeHint)
    {
        if (_position != _length)
        {
            throw new InvalidOperationException (
                "IBufferWriter implementation is only usable when position is at the end of the stream.");
        }
        ...
        var current = EnsureCurrentInitialized (sizeHint);
    ...
    internal class MemoryStreamAdapter
        : MemoryStream
    {
        ...
        {
            get => (int) _innerStream.Capacity;
            set => _innerStream.Capacity = value;
        }
        ...
        {
            return _innerStream.TryGetBuffer (out buffer);
        }
