...
public static class Algorithms
{
    ...
    private class ListRange<T> : ListBase<T>, ICollection<T>
    {
        ...
        public override bool Remove (T item)
        {
            ...
            {
                throw new NotSupportedException ("Collection is read-only and cannot be modified");
            }
    ...
    {
        return collection == null ? null : new ReadOnlyCollection<T> (collection);
    }
    ...
    public static IList<T>? ReadOnly<T> (IList<T>? list)
    {
        ...
        return list.IsReadOnly ? list : new ReadOnlyList<T> (list);
    }
        ...
        : ICollection
    {
        ...
        public void CopyTo (Array array, int index)
        {
            ...
            {
                throw new ArgumentException ("The array is too small to hold all of the items", nameof (array));
            }
    ...
    private class UntypedList<T> : IList
    {
        ...
        public void CopyTo (Array array, int index)
        {
            ...
            if (index >= array.Length || count > array.Length - index)
            {
                throw new ArgumentException ("The array is too small to hold all of the items", nameof (index));
            }
        ...
        IEqualityComparer<T> equalityComparer)
    {
        ...
        if (list.IsReadOnly)
        {
            throw new ArgumentException ("The list may not be read only.", nameof (list));
        }
    ...
    public static int FirstIndexOfMany<T> (IList<T> list, IEnumerable<T> itemsToLookFor, BinaryPredicate<T> predicate)
    {
        ...
        foreach (var x in list)
        {
            foreach (var y in itemsToLookFor)
            {
                if (predicate (x, y))
                {
                    return index;
    ...
    public static int SearchForSubsequence<T> (IList<T> list, IEnumerable<T> pattern, BinaryPredicate<T> predicate)
    {
        ...
        var patternArray = ToArray (pattern);
    ...
    private class CollectionEqualityComparer<T> : IEqualityComparer<IEnumerable<T>>
    {
        ...
        public int GetHashCode (IEnumerable<T> obj)
        {
            ...
            {
                var itemHash = Util.GetHashCode (t, equalityComparer);
                hash += itemHash;
                hash = (hash << 9) | (hash >> 23);
            }
    ...
    public static IEnumerable<T> NCopiesOf<T> (int n, T item)
    {
        if (n < 0)
        {
            throw new ArgumentOutOfRangeException (nameof (n), n, "The argument may not be less than zero");
        }
    ...
    public static void Copy<T> (IEnumerable<T> source, T[] dest, int destIndex)
    {
        ...
        using (var sourceEnum = source.GetEnumerator())
        {
            ...
            while (sourceEnum.MoveNext())
            {
                ...
                {
                    throw new ArgumentException ("The array is too small to hold all of the items", nameof (dest));
                }
    ...
    public static void Copy<T> (IEnumerable<T> source, IList<T> dest, int destIndex, int count)
    {
        ...
        if (dest.IsReadOnly)
        {
            throw new ArgumentException ("The list may not be read only", nameof (dest));
        }
