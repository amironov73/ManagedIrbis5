    ...
    : TreeNode<TNode>
{
    ...
    /// <param name="childrenPropertyName"></param>
    public TreeJsonBuilder
        (
    ...
    public TreeJsonBuilder
        ...
        : this
            ...
            n => new Dictionary<string, string>
                { { "HierarchyId", n.HierarchyId.ToString ("/").WrapDoubleQuotes() } },
    ...
    {
        return ToJson (node, _ => true, int.MaxValue, includeRoot);
    }
    ...
    {
        return ToJson (node, _ => true, maxRelativeDepth, includeRoot);
    }
    ...
    {
        if (!allowNext (root) || maxRelativeDepth < 0)
        {
            return string.Empty;
        }
        ...
        )
    {
        ...
        if (hasProperties)
        {
            _builder.Append
                (
                    propertyMap.Select (kvp =>
                        $"{kvp.Key.WrapDoubleQuotes()}:{kvp.Value}").ToCsv()
                );
        }

        BuildChildren (node, curDepth,
            $"{(hasProperties ? "," : string.Empty)}{_childrenPropertyName.WrapDoubleQuotes()}:");
