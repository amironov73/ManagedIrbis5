...
public class TreeHtmlBuilder<TNode> where TNode
    : TreeNode<TNode>
{
    ...
    public TreeHtmlBuilder
        (
            HtmlBuildDefinition<TNode>? def = null
        )
    {
        _allowNext = null!;
        _builder = null!;
    ...
    {
        return ToHtml (node, _ => true, int.MaxValue, includeRoot);
    }
    ...
    {
        return ToHtml (node, _ => true, maxRelativeDepth, includeRoot);
    }
        ...
        )
    {
        ...
        _builder.Append (
            $"<{_def.ContainerElementName}{SerializeAttributes (_def.GetContainerAttributes (node))}>");
