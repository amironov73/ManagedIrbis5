...
public static class ItemTreeBuildExtensions
{
        ...
        where TNode : ItemTreeNode<TNode, TItem>
    {
        ...
        var sourceItemsGroupedByParentId =
            sourceItems.ToLookup (getHierarchyId, HierarchyPositionParentComparer.Default);
    ...
    public static void Build<TNode, TItem, TSNode>
        ...
        where TSNode : SerialTreeNode<TSNode>, new()
    {
        if (!allowNext (sourceParent) || maxRelativeDepth <= 0)
        {
            return;
        }
    ...
    private static void Build<TNode, TItem>
        (
            this TNode parent,
            Func<TNode, IEnumerable<TItem>> getChildItems,
            int curDepth,
    ...
    private static void Build<TNode, TItem, TSNode>
        (
