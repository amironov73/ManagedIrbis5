...
/// O(1) removals from the front and back, amortized O(1) insertions
/// to the front and back, and O(N) insertions and removals anywhere
/// else (with the operations getting slower as the index approaches
/// the middle).
/// </summary>
/// <typeparam name="TItem">The type of elements contained in the deque.</typeparam>
[DebuggerDisplay ("Count = {Count}, Capacity = {Capacity}")]
[DebuggerTypeProxy (typeof (Deque<>.DebugView))]
public sealed class Deque<TItem>
    : IList<TItem>, IReadOnlyList<TItem>, IList
{
    /// <summary>
    ...
    public Deque (IEnumerable<TItem> collection)
    {
        if (collection == null)
        {
        ...
        if (count > 0)
        {
            _buffer = new TItem[count];
            DoInsertRange (0, source);
        }
        else
        {
            _buffer = new TItem[DefaultCapacity];
        }
    }
    ...
    int IList.Add (object? value)
    {
        ...
        AddToBack ((TItem)value!);
        return Count - 1;
    }

    bool IList.Contains (object? value)
    {
        return IsT (value) ? ((ICollection<TItem>)this).Contains ((TItem)value!) : false;
    }

    int IList.IndexOf (object? value)
    {
        return IsT (value) ? IndexOf ((TItem)value!) : -1;
    }
    ...
    private TItem DoRemoveFromBack()
    {
    ...
    private TItem DoRemoveFromFront()
    {
        --Count;
    ...
    public TItem[] ToArray()
    {
        ...
        ((ICollection<TItem>)this).CopyTo (result, 0);
        return result;
    }
    ...
    {
        private readonly Deque<TItem> deque;

        public DebugView (Deque<TItem> deque)
        {
            this.deque = deque;
        }
