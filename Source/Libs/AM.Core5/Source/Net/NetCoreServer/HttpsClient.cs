...
/// It allows to send GET, POST, PUT, DELETE requests and receive
/// HTTP result using secure transport.
/// </summary>
    ...
    : SslClient
{
    ...
    /// <inheritdoc cref="SslClient.OnReceived"/>
    ...
    /// <inheritdoc cref="SslClient.OnDisconnected"/>
    ...
    protected virtual void OnReceivedResponseHeader
        (
    ...
    : HttpsClient
{
    ...
    /// <param name="timeout">Current HTTP request timeout (default is 1 minute)</param>
    /// <returns>HTTP request Task</returns>
    public Task<HttpResponse> SendHeadRequest
        (
            string url,
    ...
    /// <param name="timeout">Current HTTP request timeout (default is 1 minute)</param>
    /// <returns>HTTP request Task</returns>
    public Task<HttpResponse> SendGetRequest
        (
            string url,
            TimeSpan? timeout = null
        )
    ...
    /// <inheritdoc cref="SslClient.OnHandshaked"/>
    protected override void OnHandshaked()
    {
        // Send prepared HTTP request on connect
        if (!Request.IsEmpty && !Request.IsErrorSet && !SendRequestAsync())
        {
            SetResultError ("Failed to send HTTP request!");
        }
    }

    /// <inheritdoc cref="HttpsClient.OnDisconnected"/>
    protected override void OnDisconnected()
    {
    ...
    /// <inheritdoc cref="HttpsClient.OnReceivedResponse"/>
    protected override void OnReceivedResponse
        (
            HttpResponse response
        )
    {
    ...
    /// <inheritdoc cref="HttpsClient.OnReceivedResponseError"/>
    protected override void OnReceivedResponseError
        (
            HttpResponse response,
            string error
        )
    ...
    /// <inheritdoc cref="SslClient.Dispose(bool)"/>
