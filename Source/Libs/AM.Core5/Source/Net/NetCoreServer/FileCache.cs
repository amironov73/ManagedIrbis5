...
/* FileCache.cs -- cache file content in memory
    ...
    : IDisposable
{
    ...
    {
        handler ??= (cache, key, value, timespan) =>
    ...
    private class FileCacheEntry
    {
            ...
            )
        {
            ...
            var key = eventArgs.FullPath.Replace ('\\', '/').Replace (entry._path + "/", entry._prefix);
            var file = eventArgs.FullPath.Replace ('\\', '/');
            ...
            )
        {
            ...
            var oldKey = eventArgs.OldFullPath.Replace ('\\', '/').Replace (entry._path + "/", entry._prefix);
            var oldFile = eventArgs.OldFullPath.Replace ('\\', '/');
            var newKey = eventArgs.FullPath.Replace ('\\', '/').Replace (entry._path + "/", entry._prefix);
            var newFile = eventArgs.FullPath.Replace ('\\', '/');
            oldFile.NotUsed();
    ...
    private bool InsertFileInternal
        (
            string path,
            string file,
            string key,
    ...
    private void RemoveFileInternal (string path, string key)
    {
        try
        {
            using (new WriteLock (_lockEx))
    ...
    private bool InsertPathInternal
        (
            string root,
            string path,
            string prefix,
