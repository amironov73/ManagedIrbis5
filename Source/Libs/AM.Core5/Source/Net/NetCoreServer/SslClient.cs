    ...
    : IDisposable
{
    ...
    public virtual bool Connect()
    {
        ...
        try
        {
            ...
            {
                _sslStream.AuthenticateAsClient (Address, new X509CertificateCollection (new[] { Context.Certificate }),
                    Context.Protocols, true);
            }
            else
            {
    ...
    public virtual bool Disconnect()
    {
        ...
        if (IsConnecting)
        {
            if (_connectEventArg is not null)
            {
    ...
    private void TryReceive()
    {
        ...
        if (_sslStream is null || _receiveBuffer is null)
        {
            throw new ArsMagnaException();
        }
        ...
        )
    {
        ...
        if (Socket is null || _receiveBuffer is null
            || _sendBufferMain is null|| _sendBufferFlush is null)
        {
            throw new ArsMagnaException();
        }
        ...
        if (eventArgs.SocketError == SocketError.Success)
        {
            ...
            try
            {
                ...
                {
                    _sslStream.BeginAuthenticateAsClient (Address,
                        new X509CertificateCollection (new[] { Context.Certificate }), Context.Protocols, true,
                        ProcessHandshake, _sslStreamId);
                }
                else
                {
    ...
    private void ProcessHandshake (IAsyncResult result)
    {
        try
        {
            ...
            if (_sslStream is null || _sendBufferMain is null)
            {
                throw new ArsMagnaException();
            }
    ...
    private void ProcessSend (IAsyncResult result)
    {
        try
        {
            ...
            if (_sslStream is null || _sendBufferFlush is null)
            {
                throw new ArsMagnaException();
            }
