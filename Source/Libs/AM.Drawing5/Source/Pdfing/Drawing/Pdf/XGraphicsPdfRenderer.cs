...
/// </summary>
internal class XGraphicsPdfRenderer
    : IXGraphicsRenderer
{
    #region Construction
    ...
    public void DrawString (string s, XFont font, XBrush brush, XRect rect, XStringFormat format)
    {
        ...
        var boldSimulation = (font.GlyphTypeface.StyleSimulations & XStyleSimulations.BoldSimulation) != 0;
        var strikeout = (font.Style & XFontStyle.Strikeout) != 0;
        var underline = (font.Style & XFontStyle.Underline) != 0;
        ...
        if (underline)
        {
            var underlinePosition = lineSpace * realizedFont.FontDescriptor._descriptor.UnderlinePosition /
                                    font.CellSpace;
            var underlineThickness = lineSpace * realizedFont.FontDescriptor._descriptor.UnderlineThickness /
                                     font.CellSpace;

            //DrawRectangle(null, brush, x, y - underlinePosition, width, underlineThickness);
            var underlineRectY = Graphics.PageDirection == XPageDirection.Downwards
                ? y - underlinePosition
        ...
        if (strikeout)
        {
            var strikeoutPosition = lineSpace * realizedFont.FontDescriptor._descriptor.StrikeoutPosition /
                                    font.CellSpace;
            var strikeoutSize =
            ...
            var strikeoutRectY = Graphics.PageDirection == XPageDirection.Downwards
                ? y - strikeoutPosition
    ...
    internal string GetFormName (XForm form)
    {
        return _page is not null
            ? _page.GetFormName (form)
            : _form.ThrowIfNull().GetFormName (form);
    }
