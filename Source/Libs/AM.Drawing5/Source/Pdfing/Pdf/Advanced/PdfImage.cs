...
public sealed partial class PdfImage : PdfXObject
{
    ...
    private void ReadTrueColorMemoryBitmap (int components, int bits, bool hasAlpha)
    {
        ...
        if (streamLength > 0)
        {
            ...
            var infoHeaderSize = ReadDWord (imageBits, 14); // sizeof BITMAPINFOHEADER
    ...
    */
    ...
    private void ReadIndexedMemoryBitmap (int bits /*, ref bool hasAlpha*/)
    {
        ...
        if (streamLength > 0)
        {
            ...
            var fileBits = ReadWord (imageBits, 28);
            if (fileBits != bits)
            {
            ...
            var bytesFileOffset = ReadDWord (imageBits, 10);
            const int
                bytesColorPaletteOffset =
                    0x36; // GDI+ always returns Windows bitmaps: sizeof BITMAPFILEHEADER + sizeof BITMAPINFOHEADER
            var paletteColors = ReadDWord (imageBits, 46);
            ...
            if (bits == 1)
            {
                ...
                var ccittSizeG4 = DoFaxEncodingGroup4 (ref tempG4, imageBits, (uint)bytesFileOffset, (uint)width,
                    (uint)height);
                    ...
                    for (var y = 0; y < height; ++y)
                    {
                        ...
                        for (var x = 0; x < bytesPerLine; ++x)
                        {
                            if (isGray)
                            {
                                ...
                                imageData[bytesOffsetWrite] =
                                    paletteData[3 * imageBits[bytesFileOffset + bytesOffsetRead]];
                            }
                            else
                            {
                ...
                (!usesCcittEncoding && isBitonal <= 0 && !isGray))
            {
                ...
                if (packedPaletteData != null &&
                    packedPaletteData.Length + 20 <
                    paletteData.Length) // +20: compensate for the overhead (estimated value)
                {
