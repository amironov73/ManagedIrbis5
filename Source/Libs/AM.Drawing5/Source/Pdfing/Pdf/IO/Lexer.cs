...
public class Lexer
{
    ...
    public byte[] ReadStream (int length)
    {
        ...
        var read = _pdfSteam.Read (bytes, 0, length);
    ...
    public Symbol ScanHexadecimalString()
    {
        ...
        while (true)
        {
            ...
            if (IsHexChar (_currChar))
            {
                ...
                var ch = int.Parse (new string (hex), NumberStyles.HexNumber);
...
// #endif
    ...
    /// </summary>
    public string Token => _token?.ToString() ?? string.Empty;
