...
internal class PdfWriter
{
    ...
    public void Write (PdfRectangle rect)
    {
        ...
        WriteSeparator (CharCat.Delimiter, '/');
        WriteRaw (PdfEncoders.Format (
            "[{0:" + format + "} {1:" + format + "} {2:" + format + "} {3:" + format + "}]", rect.X1, rect.Y1,
            rect.X2, rect.Y2));
        _lastCat = CharCat.Delimiter;
    }
    ...
    public void WriteEof (PdfDocument document, long startxref)
    {
        ...
        if (_layout == PdfWriterLayout.Verbose)
        {
            ...
            WriteRaw ("Creation time: " + duration.TotalSeconds.ToString ("0.000", CultureInfo.InvariantCulture) +
                      " seconds");
