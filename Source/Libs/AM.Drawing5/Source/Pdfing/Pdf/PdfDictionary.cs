...
public class PdfDictionary
    : PdfObject, IEnumerable<KeyValuePair<string, PdfItem>>
{
    ...
    {
        get { return _elements ??= new DictionaryElements (this); }
    }
    ...
    internal override void WriteObject (PdfWriter writer)
    {
        ...
        if (Stream != null)
        {
            Debug.Assert (Elements.ContainsKey (PdfStream.Keys.Length),
                "Dictionary has a stream but no length is set.");
        }
#endif
    ...
    public sealed class DictionaryElements
        : IDictionary<string, PdfItem>, ICloneable
    {
                ...
                if ((iref = value as PdfReference) != null)
                {
                    ...
                    if (true) // || _owner.Document.IsImported)
                    {
                        ...
                        Debug.Assert (type != null,
                            "No value type specified in meta information. Please send this file to PDFsharp support.");
                        ...
                        if (type != null && type != value.GetType())
                        {
                            if (typeof (PdfDictionary).GetTypeInfo().IsAssignableFrom (typeInfo))
                            {
                                Debug.Assert (value is PdfDictionary,
                                    "Bug in PdfSharpCore. Please send this file to PDFsharp support.");
        ...
        PdfArray CreateArray (Type type, PdfArray oldArray)
        {
            ...
            if (oldArray == null)
            {
                ...
                var ctorInfos =
                    type.GetTypeInfo()
                        .DeclaredConstructors; //.GetConstructor(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic,
        ...
        PdfDictionary CreateDictionary (Type type, PdfDictionary oldDictionary)
        {
            ...
            else
            {
                var ctorInfos =
                    type.GetTypeInfo()
                        .DeclaredConstructors; // GetConstructor(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, null, new Type[] { typeof(PdfDictionary) }, null);
                foreach (var ctorInfo in ctorInfos)
                {
                    var parameters = ctorInfo.GetParameters();
