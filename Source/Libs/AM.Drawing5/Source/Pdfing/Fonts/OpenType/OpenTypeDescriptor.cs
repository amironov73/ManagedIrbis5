...
namespace PdfSharpCore.Fonts.OpenType
{
    ...
    internal sealed class OpenTypeDescriptor : FontDescriptor
    {
        ...
        ///
        /// </summary>
        /// <param name="fontDescriptorKey"></param>
        /// <param name="idName"></param>
        /// <param name="fontData"></param>
        internal OpenTypeDescriptor
            (
                string fontDescriptorKey,
                string idName,
                byte[] fontData
        ...
        void Initialize()
        {
            ...
            var os2SeemsToBeEmpty = FontFace._os2.sTypoAscender == 0 && FontFace._os2.sTypoDescender == 0 &&
                                    FontFace._os2.sTypoLineGap == 0;

            //Debug.Assert(!os2SeemsToBeEmpty); // Are there fonts without OS/2 table?

            var dontUseWinLineMetrics = (FontFace._os2.fsSelection & 128) != 0;
            ...
            else
            {
                ...
                if (!os2SeemsToBeEmpty)
                {
                    ...
                    var winDescent = Math.Abs (FontFace._os2.usWinDescent!);
            ...
            var symbol = FontFace._cmap!.symbol;
            Widths = new int[256];
        ...
        public override bool IsBoldFace =>
            ...
            FontFace._os2!.IsBold;

        /// <summary>
        /// Gets a value indicating whether this instance belongs to an italic font.
        /// </summary>
        public override bool IsItalicFace => FontFace._os2!.IsItalic;
        ...
        public int CharCodeToGlyphIndex (char value)
        {
            try
            {
                var cmap4 = FontFace._cmap!.cmap4;
        ...
        ///   //Converts the width of a glyph identified by its index to PDF design units.
        /// </summary>
        public double GlyphIndexToEmfWidth
            (
                int glyphIndex,
                double emSize
            )
