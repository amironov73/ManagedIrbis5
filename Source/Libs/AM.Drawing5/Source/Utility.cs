...
public static class Utility
{
    ...
    /// <summary>
    /// Изображение в ландшафтной ориентации?
    /// </summary>
    ...
    /// <returns></returns>
    public static Image LoadFromMemory
        (
            byte[] bytes
        )
    {
    ...
    {
        var arrayOfPoints = Points (points);
        if (!ReferenceEquals (brush, null))
        {
    ...
    {
        var width1 = corners.Width;
        var height1 = corners.Height;
        var width2 = width1 / 2;
        ...
        )
    {
        using (var path = new GraphicsPath())
        {
            path.AddString (text,
                ...
                rect,
                format);
            using (var tempPen = new Pen (Color.Black, width))
            {
        ...
        )
    {
        ...
        var result = new Point[values.Length / 2];
        for (int i = 0,
             j = 0;
             i < result.Length;
             i++)
        ...
        )
    {
        ...
        var inches = Math.Min (size.Width / g.DpiX, size.Height / g.DpiY);
        ...
        )
    {
        using (var path = new GraphicsPath())
        {
            ...
            {
                var lightColor = Utility.Lighten (color, 0.8f);
                var darkColor = Utility.Darken (color, 0.6f);
                var blend = new ColorBlend
                {
                    Colors = new[]
                    {
        ...
        )
    {
        ...
        bottom.Y += height;
        var rx = rectangle.Width / 2f;
        var ry = rectangle.Height / 2f;
        var cx = rectangle.Left + rx;
        var cy = rectangle.Top + ry;
        var lighterColor = Utility.Lighten (color, 0.5f);
        var darkerColor = Utility.Darken (color, 0.5f);
                ...
                using (Brush darkerBrush = new SolidBrush (darkerColor))
                {
                               ...
                               0f))
                    {
                        using (var arcPath = new GraphicsPath())
                        {
                            ...
                            var topCenter = new PointF (cx, cy);
                            var bottomCenter = new PointF (cx, cy + height);
                            var topStart = pathPoints[1];
                            var bottomStart = topStart;
                            bottomStart.Y += height;
                            var topEnd = pathPoints[pathPoints.Length - 1];
                            var bottomEnd = topEnd;
                            bottomEnd.Y += height;

                            var startEdge = new PointF[5];
                            ...
                            var angle2 = Math.Min (endAngle, 180f);
                            if ((angle1 <= 180)
                                || (angle2 <= 180)
                               )
                            {
                                using (var path = new GraphicsPath())
                                {
                                    ...
                                    var bottomLast = _LastPoint (path);
                                    var topLast = bottomLast;
                                    topLast.Y -= height;
