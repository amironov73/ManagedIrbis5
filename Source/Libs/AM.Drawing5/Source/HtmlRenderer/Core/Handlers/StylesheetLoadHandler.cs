...
internal static class StylesheetLoadHandler
{
    ...
    /// First raise <see cref="HtmlStylesheetLoadEventArgs"/>
    /// event to allow the client to overwrite the stylesheet loading.<br/>
    /// If the stylesheet is downloaded from URI we will try
    /// to correct local URIs to absolute.<br/>
    /// </summary>
    ...
    /// <param name="stylesheet">return the stylesheet string that
    /// has been loaded (null if failed or <paramref name="stylesheetData"/> is given)</param>
    /// <param name="stylesheetData">return stylesheet data object
    /// that was provided by overwrite (null if failed or <paramref name="stylesheet"/> is given)</param>
    ...
    private static string LoadStylesheetFromFile
        (
            HtmlContainerInt htmlContainer,
            string path
        )
    ...
    /// <returns>the loaded stylesheet string</returns>
    private static string LoadStylesheetFromUri
        (
            HtmlContainerInt htmlContainer,
            Uri uri
        )
    ...
    private static string CorrectRelativeUrls
        (
            string stylesheet,
            Uri baseUri
        )
