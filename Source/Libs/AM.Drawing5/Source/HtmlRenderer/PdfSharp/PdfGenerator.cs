...
namespace AM.Drawing.HtmlRenderer.PdfSharp;
...
public static class PdfGenerator
{
    ...
    {
        Sure.NotNull (fromFamily);
        Sure.NotNull (toFamily);
    ...
    /// <param name="imageLoad">optional: can be used to overwrite image resolution logic</param>
    /// <returns>the generated image of the html</returns>
    public static void AddPdfPages
        (
        ...
        )
    {
        ...
        var pageSize = new XSize (orgPageSize.Width - config.MarginLeft - config.MarginRight,
            orgPageSize.Height - config.MarginTop - config.MarginBottom);
        ...
        if (!string.IsNullOrEmpty (html))
        {
            ...
            using (var measure =
                   XGraphics.CreateMeasureContext (pageSize, XGraphicsUnit.Point, XPageDirection.Downwards))
            {
        ...
        )
    {
        foreach (var link in container.GetLinks())
        {
            ...
            for (; i < document.Pages.Count && pageSize.Height * i < link.Rectangle.Bottom; i++)
            {
                ...
                var xRect = new XRect (link.Rectangle.Left,
                    orgPageSize.Height - (link.Rectangle.Height + link.Rectangle.Top - offset), link.Rectangle.Width,
