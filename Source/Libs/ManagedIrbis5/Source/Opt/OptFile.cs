    ...
    : IHandmadeSerializable
{
        ...
        )
    {
        ...
        while (true)
        {
            var line = reader.RequireLine().Trim();
            if (line.StartsWith ("*"))
            {
                break;
            }
        ...
        )
    {
        foreach (var line in Lines)
        {
            ...
            {
                return line.Value.ThrowIfNull();
            }
        }

        Magna.Logger.LogError
            (
                nameof (OptFile) + "::" + nameof (SelectWorksheet)
                + ": can't select worksheet for {Tag}",
        ...
        )
    {
        ...
        foreach (var line in Lines)
        {
            writer.WriteLine
                (
                    "{0} {1}",
                    line.Key.ThrowIfNull().PadRight (WorksheetLength),
                    line.Value
                );
        }
        ...
        )
    {
        if (length <= 0)
        {
            Magna.Logger.LogError
                (
                    nameof (OptFile) + "::" + nameof (SetWorksheetLength)
                    + ": length={Length}",
                    length
                );
        ...
        )
    {
        ...
        {
            result = Lines
                         .GroupBy (item => item.Key.ThrowIfNull().ToUpper())
