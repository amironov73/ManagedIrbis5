...
internal static class XmlUtils
{
    ...
    private static void SkipXmlHeader (MemoryStream memoryStream)
    {
        ...
        {
            for (var i = startPosition; i < buffer.Count - pattern.Length; i++)
            {
        ...
        memoryStream.TryGetBuffer (out var memoryStreamBuffer);
        var position = IndexOf (memoryStreamBuffer, XML_DECLARATION_FIRST_CHARS, 0);
        if (position == -1)
        {
            return;
        }
