...
/* PftFirst.cs -- функция, выдает номер первого повторения поля, для которого выполняется условие
    ...
    : PftNumeric
{
    ...
    public override IList<PftNode> Children
    {
        ...
        protected set
        {
            ...
            Magna.Logger.LogError
                (
                    nameof (PftFirst) + "::" + nameof (Children)
                    + ": set value={Value}",
        ...
        )
    {
        ...
        if (context.CurrentGroup is not null)
        {
            Magna.Logger.LogError
                (
                    nameof (PftFirst) + "::" + nameof (Execute)
                    + ": nested group detected at {Token}",
                    this
                );

            throw new PftSemanticException ("Nested group detected at " + this);
        }
