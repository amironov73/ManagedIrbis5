    ...
    IDisposable
{
        ...
        )
    {
        ...
        Driver = parent is null
            ? new PlainTextDriver (Output)
            : parent.Driver;

        Globals = parent is null
            ? new PftGlobalManager()
            : parent.Globals;

        Variables = parent is null
            ? new PftVariableManager (null)
            : parent.Variables;

        Procedures = parent is null
        ...
        )
    {
        ...
        Magna.Logger.LogTrace (nameof (PftContext) + "::" + nameof (ActivateDebugger));
