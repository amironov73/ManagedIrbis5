...
/* TypeMap.cs -- карта типов для сериализации AST
...
/// <summary>
/// Карта типов для сериализации AST.
/// </summary>
        ...
        foreach (var entry in Map)
        {
            ...
            if (ReferenceEquals (constructor, null))
            {
                Magna.Logger.LogError
                    (
                        nameof (TypeMap) + "::StaticConstructor"
                        + ": can't find constructor for {Type}",
                        entry.Type
                    );

                throw new IrbisException ($"Can't find constructor for {entry.Type}");
            }
