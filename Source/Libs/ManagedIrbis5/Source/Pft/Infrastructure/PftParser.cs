...
/* PftParser.cs -- парсер PFT-скриптов
...
public sealed partial class PftParser
{
    ...
    private PftA ParseA()
    {
        ...
        if (field.Command != 'v' && field.Command != 'g')
        {
            Magna.Logger.LogError
                (
                    nameof (PftParser) + "::" + nameof (ParseA)
                    + ": bad field specified"
                );
        ...
        try
        {
            ...
            try
            {
                ...
                if (!Tokens.IsEof)
                {
                    Magna.Logger.LogError
                        (
                            nameof (PftParser) + "::" + nameof (ParseAssignment)
                            + ": garbage detected"
                        );
    ...
    private PftNode ParseEat()
    {
        ...
        if (!ok)
        {
            Magna.Logger.LogError
                (
                    nameof (PftParser) + "::" + nameof (ParseEat)
                    + ": syntax error"
                );
    ...
    private PftNode ParseF()
    {
        ...
        if (Tokens.IsEof)
        {
            Magna.Logger.LogError
                (
                    nameof (PftParser) + "::" + nameof (ParseF)
    ...
    private PftNode ParseF2()
    {
           ...
           )
        {
            Magna.Logger.LogError
                (
                    nameof (PftParser) + "::" + nameof (ParseF2)
                    + ": syntax error"
                );
    ...
    private PftNode ParseGroup()
    {
        ...
        if (_inGroup)
        {
            Magna.Logger.LogError
                (
                    nameof (PftParser) + "::" + nameof (ParseGroup)
    ...
    private PftHave ParseHave()
    {
        ...
        else
        {
            Magna.Logger.LogError
                (
                    nameof (PftParser) + "::" + nameof (ParseHave)
                    + ": syntax error"
                );
    ...
    private PftNode ParseLocal()
    {
        ...
        Tokens.MoveNext();
        var localTokens = Tokens.Segment (_doStop).ThrowIfNull ("localTokens");
        Tokens.MoveNext();
        ...
        while (!localTokens.IsEof)
        {
            ...
            switch (token.Kind)
            {
                ...
                default:
                    Magna.Logger.LogError
                        (
                            nameof (PftParser) + "::" + nameof (ParseLocal)
    ...
    private PftP ParseP()
    {
        ...
        if (ReferenceEquals (field, null))
        {
            Magna.Logger.LogError
                (
                    nameof (PftParser) + "::" + nameof (ParseP)
                    + ": field not set"
                );
    ...
    private PftNode ParseProc()
    {
        ...
        if (_inProcedure)
        {
            Magna.Logger.LogError
                (
                    nameof (PftParser) + "::" + nameof (ParseProc)
        ...
        try
        {
            ...
            var name = procedure.Name.ThrowIfNull();
            if (name.IsOneOf (PftUtility.GetReservedWords()))
            {
                ...
                throw new PftSyntaxException ("reserved word: " + name);
            }
               ...
               )
            {
                Magna.Logger.LogError
                    (
                        nameof (PftParser) + "::" + nameof (ParseProc)
                        + ": already have function {Function}",
                        name.ToVisibleString()
                    );
            ...
            if (!ReferenceEquals (_procedures.FindProcedure (name), null))
            {
                Magna.Logger.LogError
                    (
                        nameof (PftParser) + "::" + nameof (ParseProc)
                        + ": already have procedure {Procedure}",
                        name.ToVisibleString()
                    );

                throw new PftSyntaxException ("already have procedure: " + name);
            }

            _procedures.Registry.Add (name, procedure);
    ...
    private PftNode ParseVariable()
    {
        ...
        if (Tokens.Peek() == PftTokenKind.Equals)
        {
            if (_inAssignment)
            {
                Magna.Logger.LogError
                    (
                        nameof (PftParser) + "::" + nameof (ParseVariable)
        ...
        while (!fieldTokens.IsEof)
        {
            ...
            switch (token.Kind)
            {
                case PftTokenKind.V:
                    ...
                    if (!specification.ParseShort (token.Text.ThrowIfNull ()))
                    {
                        Magna.Logger.LogError
                            (
                                nameof (PftParser) + "::" + nameof (ParseWith)
                                + ": field not specified"
                            );
