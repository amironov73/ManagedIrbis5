...
/* Marc8Encoding.cs -- кодировка MARC8
...
/*
  ...
  The MARC formats are standards for the representation and communication
  of bibliographic and related information in machine-readable form,
  and they are frequently used in library database systems. The character
  encoding now known as MARC-8 was introduced in 1968 as part of the MARC
  format. Originally based on the Latin alphabet, from 1979 to 1983 the
  JACKPHY initiative expanded the repertoire to include Japanese, Arabic,
  Chinese, and Hebrew characters (among others), with the later addition
  of Cyrillic and Greek scripts. If a character is not representable
  in MARC-8 of a MARC-21 record, then UTF-8 must be used instead.
  UTF-8 has support for many more characters than MARC-8, which is rarely
  used outside library data.

  MARC-8 uses a variant of the ISO-2022 encoding. It uses escape characters
  to represent characters beyond the 7-bit ASCII range of characters.

  It generally uses the same logical BiDi ordering as Unicode.

  The combining characters and base characters are in a different order than
  used in Unicode. The following are some examples. The combining characters
  are not always stored in reverse order as Unicode normalization.
  The MARC-21 standard describes the MARC-8 Unicode conversion issues
  in more detail.
...
/// Кодировка MARC8.
/// </summary>
public sealed class Marc8Encoding
    : Encoding
{
    #region Encoding members

    /// <inheritdoc cref="Encoding.EncodingName"/>
    public override string EncodingName => "MARC8";

    /// <inheritdoc cref="Encoding.GetByteCount(char[],int,int)"/>
    ...
    /// <inheritdoc cref="Encoding.GetBytes(char[],int,int,byte[],int)"/>
        ...
        )
    {
        ...
        for (var i = 0; i < charCount; i++)
        {
            ...
            var convertedBytes = ConvertToMARC8Bytes (chars[charIndex + i]);
    ...
    ///<inheritdoc cref="Encoding.GetCharCount(byte[],int,int)"/>
    ...
    /// <inheritdoc cref="Encoding.GetChars(byte[],int,int,char[],int)"/>
        ...
        )
    {
        ...
        for (var i = 0; i < byteCount; i++)
        {
            ...
            var charsAppended = AppendUnicodeCharacter (chars, writeCharIndex, marcByte1, marcByte2, marcByte3);
            if (charsAppended > 0)
            {
    ...
    /// <inheritdoc cref="Encoding.GetMaxByteCount"/>
    public override int GetMaxByteCount
        (
            int charCount
        )
    {
        Sure.NonNegative (charCount);

        return charCount * 3;
    }

    /// <inheritdoc cref="Encoding.GetMaxCharCount"/>
    public override int GetMaxCharCount
        (
            int byteCount
        )
    {
        Sure.NonNegative (byteCount);
    ...
    /// Based on code from Mark V. Sullivan's SobekCM MARC Library
    /// (https://sourceforge.net/projects/marclibrary/) which is GPLv3
    /// </summary>
    ...
    /// <returns></returns>
    private static int AppendUnicodeCharacter
        (
            char[] chars,
            int charIndex,
            int marcByte1,
        ...
        )
    {
        ...
        else //This is a THREE byte combination
        {
            ...
            var fallbackEncoding = UTF8.GetString (new[] { (byte)marcByte3, (byte)marcByte2, (byte)marcByte1 });
            foreach (var c in fallbackEncoding)
            {
                chars[charIndex] = c;
                charIndex++;
    ...
    private static byte?[] ConvertToMARC8Bytes
        (
            char c
        )
    {
        var bytes = new byte?[] { null, null, null };
