...
/* IfpRecord64.cs -- запись в инвертированном файле ИРБИС64
...
/// <summary>
/// Запись в инвертированном файле ИРБИС64.
/// </summary>
public sealed class IfpRecord64
{
    ...
    /// <summary>
    /// ibatrak Структура с описанием специального блока.
    /// </summary>
    private class SpecialPostingBlockEntry
    {
        ...
        public long Offset
        {
            get => unchecked ((uint) High << 4) | (uint) Low;
            set
            {
                unchecked
                {
        ...
        {
            return $"Offset:{Offset}, Posting: {Posting}";
        }
    }
    ...
    /// Собственно ссылки.
    /// </summary>
    public List<TermLink> Links { get; } = new ();
    ...
    /// <summary>
    /// Чтение IFP-записи из указанного потока.
    /// </summary>
        ...
        )
    {
        ...
        if (result.LowOffset == -1001 && result.HighOffset == -1001)
        {
            ...
            var entry = new SpecialPostingBlockEntry
            {
            ...
            for (var i = 0; i < result.BlockLinkCount; i++)
            {
