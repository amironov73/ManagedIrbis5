    ...
    public abstract partial class PictureObjectBase : ReportComponentBase
    {
        ...
        public string ImageSourceExpression
        {
            ...
            set
            {
                ...
                {
                    var expression = ImageSourceExpression;
        ...
        /// </summary>
        [Browsable (false)]
        public bool IsDataColumn => !string.IsNullOrEmpty (DataColumn);
        ...
        public override void Assign (Base source)
        {
            ...
            if (source is PictureObjectBase src)
            {
        ...
        public void GetImageAngleTransform (RectangleF drawRect, float imageWidth, float imageHeight, float scaleX,
            float scaleY, float offsetX, float offsetY, out PointF upperLeft, out PointF upperRight,
            out PointF lowerLeft)
        {
            var rect = drawRect;
            switch (SizeMode)
            {
            ...
            var gridCompensationX = offsetX + rect.X;
            gridCompensationX = (int)gridCompensationX - gridCompensationX;
            var gridCompensationY = offsetY + rect.Y;
            ...
            switch (SizeMode)
            {
                case PictureBoxSizeMode.Normal:
                {
                    ...
                    var ur = rotateVector (upperRight, angle);
                    var ll = rotateVector (lowerLeft, angle);
                ...
                case PictureBoxSizeMode.CenterImage:
                {
                    ...
                    var w = rect.Left - (drawRect.Left + drawRect.Width / 2);
                    var h = rect.Top - (drawRect.Top + drawRect.Height / 2);
                ...
                case PictureBoxSizeMode.Zoom:
                {
                    ...
                    var center = new PointF (drawRect.Left + drawRect.Width / 2,
        ...
        private void UpdateAlign (RectangleF drawRect, ref PointF upperLeft, ref PointF upperRight,
            ref PointF lowerLeft)
        {
            var lowerRight = new PointF (upperRight.X + lowerLeft.X - upperLeft.X,
                upperRight.Y + lowerLeft.Y - upperLeft.Y);
            var top = Math.Min (Math.Min (upperLeft.Y, Math.Min (upperRight.Y, lowerLeft.Y)), lowerRight.Y);
            var botom = Math.Max (Math.Max (upperLeft.Y, Math.Max (upperRight.Y, lowerLeft.Y)), lowerRight.Y);
            var height = botom - top;
            var offsetY = drawRect.Y - top;

            var left = Math.Min (Math.Min (upperLeft.X, Math.Min (upperRight.X, lowerLeft.X)), lowerRight.X);
            var right = Math.Max (Math.Max (upperLeft.X, Math.Max (upperRight.X, lowerLeft.X)), lowerRight.X);
            var width = right - left;
            var offsetX = drawRect.X - left;
        ...
        {
            var angle = (float)(fangle / 180.0 * Math.PI);
            var ax = p.X;
            var ay = p.Y;

            var bx = ax * (float)Math.Cos (angle) - ay * (float)Math.Sin (angle);
            var by = ax * (float)Math.Sin (angle) + ay * (float)Math.Cos (angle);
        ...
        internal virtual void DrawImageInternal (FRPaintEventArgs e, RectangleF drawRect)
        {
            ...
            GetImageAngleTransform (drawRect, imageWidth, imageHeight, e.ScaleX, e.ScaleY, matrix.OffsetX,
                matrix.OffsetY, out var upperLeft, out var upperRight, out var lowerLeft);
        ...
        protected abstract void DrawImageInternal2 (IGraphics graphics, PointF upperLeft, PointF upperRight,
            PointF lowerLeft);
