...
namespace AM.Reporting.Export
{
    ...
    public static class ExportUtils
    {
        ...
        {
            value = 0;
            if (format is NumberFormat numberFormat)
            {
                return decimal.TryParse (text, NumberStyles.Number, numberFormat.GetNumberFormatInfo(),
                    out value);
            }
            else if (format is CurrencyFormat currencyFormat)
            {
        ...
        internal static bool ParseTextToDateTime (string text, FormatBase format, out DateTime value)
        {
            ...
            {
                return DateTime.TryParse (text, CultureInfo.CurrentCulture.DateTimeFormat, DateTimeStyles.None,
                    out value);
            }
        ...
        internal static bool ParseTextToPercent (string text, FormatBase format, out decimal value)
        {
            ...
            {
                var returned = decimal.TryParse (text.Replace ("%", ""), out value);
                value /= 100;
                return returned;
            }
        ...
        internal static string GetExcelFormatSpecifier (FormatBase format, bool useLocale = false,
            bool currencyToAccounting = false)
        {
            if (format is CurrencyFormat currencyFormat)
            {
                var f = currencyFormat.GetNumberFormatInfo();

                if (useLocale)
                {
                    currencyFormat.UseLocale = true;
                    f = currencyFormat.GetNumberFormatInfo();
                ...
                var negative_pattern = "";

                switch (f.CurrencyPositivePattern)
                {
                    case 0:
                        positive_pattern = currency_symbol + fm_str;
                        break; // $n
                    case 1:
                        positive_pattern = fm_str + currency_symbol;
                        break; // n$
                    case 2:
                        positive_pattern = currency_symbol + " " + fm_str;
                        break; // $ n
                ...
                {
                    case 0:
                        negative_pattern = "(" + currency_symbol + fm_str + ")";
                        break; // ($n)
                    case 1:
                        negative_pattern = "-" + currency_symbol + fm_str;
                        break; // -$n
                    ...
                    case 4:
                        ...
                        break; // (n$)
                    case 5:
                        negative_pattern = "-" + fm_str + currency_symbol;
                        break; // -n$
                    case 6:
                        negative_pattern = fm_str + "-" + currency_symbol;
                        break; // n-$
                ...
                if (currencyToAccounting)
                {
                    ...
                    {
                        case 0:
                            positive_pattern = @"_-* " + financeSymbol + fm_str + "_-;";
                            break; // $n
                        case 1:
                            positive_pattern = @"_-* " + fm_str + financeSymbol + "_-;";
                            break; // n$
                        case 2:
                            positive_pattern = @"_-* " + financeSymbol + " " + fm_str + "_-;";
                            break; // $ n
                    ...
                    {
                        case 0:
                            negative_pattern = @"_-* " + "(" + financeSymbol + fm_str + ")" + "_-;";
                            break; // ($n)
                        case 1:
                            negative_pattern = @"_-* " + "-" + financeSymbol + fm_str + "_-;";
                            break; // -$n
                        ...
                        case 3:
                            ...
                            break; // $n-
                        case 4:
                            negative_pattern = @"_-* " + "(" + fm_str + financeSymbol + ")" + "_-;";
                            break; // (n$)
                        case 5:
                            negative_pattern = @"_-* " + "-" + fm_str + financeSymbol + "_-;";
                            break; // -n$
                    ...
                    var financeFormat_Options = @"_-* &quot;-&quot;??\ " + financeSymbol + "_-;_-@_-";
            ...
            else if (format is NumberFormat numberFormat)
            {
                var f = numberFormat.GetNumberFormatInfo();

                if (useLocale)
                {
                    numberFormat.UseLocale = true;
                    f = numberFormat.GetNumberFormatInfo();
            ...
            else if (format is DateFormat dateFormat)
            {
                var parentalCase = CultureInfo.CurrentCulture.TwoLetterISOLanguageName == "ru" ? "[$-FC19]" : "";
                switch (dateFormat.Format)
                {
                    case "d": return DateTimeFormatInfo.CurrentInfo.ShortDatePattern + ";@";
                    case "D":
                        return "[$-F800]" + DateTimeFormatInfo.CurrentInfo.LongDatePattern.Replace ("tt", "AM/PM") +
                               ";@";
                    case "f":
                        return parentalCase +
                               (DateTimeFormatInfo.CurrentInfo.LongDatePattern + " " +
                                DateTimeFormatInfo.CurrentInfo.ShortTimePattern).Replace ("tt", "AM/PM") + ";@";
                    case "F":
                        return parentalCase +
                               DateTimeFormatInfo.CurrentInfo.FullDateTimePattern.Replace ("tt", "AM/PM") + ";@";
                    case "MMMM yyyy": return dateFormat.Format + ";@";
                    default: return parentalCase + dateFormat.Format.Replace ("tt", "AM/PM") + ";@";
                }
            }
            else if (format is PercentFormat percentFormat)
            {
                var pattern = "0";

                if (useLocale)
                {
                    percentFormat.UseLocale = true;
                    percentFormat.DecimalDigits =
                        CultureInfo.CurrentCulture.NumberFormat.PercentDecimalDigits;
                }

                if (percentFormat.DecimalDigits > 0)
                {
                    pattern += ".";
        ...
        internal static string UInt16Tohex (ushort word)
        {
            var sb = new FastString (4);
            return sb.Append (ByteToHex ((byte)((word >> 8) & 0xFF))).Append (ByteToHex ((byte)(word & 0xFF)))
                .ToString();
        }
        ...
        {
            var Result = new FastString (text.Length);
            var len = text.Length;
        ...
        {
            var sb = new FastString ((int)(Values.Length * 1.3));
            var length = 0;
        ...
        internal static string HtmlURL (string Value)
        {
            ...
            for (var i = 0; i < Value.Length; i++)
            {
                switch (Value[i])
                {
                    ...
                    case '\'':
                    case '"':
                        Result.Append ("%" + ByteToHex ((byte)Value[i]));
                        break;
                    default:
        ...
        internal static void ZLibDeflate (Stream src, Stream dst)
        {
            ...
            src.Position = 0;
            var adler = 1L;
            using (var compressor = new DeflateStream (dst, CompressionMode.Compress, true))
            {
        ...
        {
            var sb = new FastString (s.Length * 2);
            foreach (var c in s)
            {
        ...
        {
            var result = new FastString (array.Length);
        ...
        {
            if (Fill is SolidFill fill)
            {
                return fill.Color;
            }
            else if (Fill is GlassFill glassFill)
            {
                return glassFill.Color;
            }
            else if (Fill is HatchFill hatchFill)
            {
                return hatchFill.BackColor;
            }
            else if (Fill is PathGradientFill gradientFill)
            {
                return gradientFill.CenterColor;
            }
            else if (Fill is LinearGradientFill linearGradientFill)
            {
                return GetMiddleColor (linearGradientFill.StartColor, linearGradientFill.EndColor);
            }
            else
            {
                return Color.White;
        ...
        internal static string GetRFCDate (DateTime datetime)
        {
            ...
            var hours = TimeZone.CurrentTimeZone.GetUtcOffset (datetime).Hours;
            var minutes = TimeZone.CurrentTimeZone.GetUtcOffset (datetime).Minutes;
            ...
            {
                var offset = (hours >= 0 && minutes >= 0 ? "+" : "") + hours.ToString ("00") +
                             minutes.ToString ("00");
        ...
        {
            var cellReference = new StringBuilder (4);
            ...
            do
            {
                ...
                var digit = x % MAX_CHARS;
                var cellDig = (char)((char)'A' + (char)digit);
                cellReference.Append (cellDig);
                x -= digit;
                x /= (MAX_CHARS);
            } while (x > 0);

            for (var i = 0; i < cellReference.Length / 2; i++)
            {
