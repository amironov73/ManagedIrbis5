    ...
    public partial class StimulSoftImport : ImportBase
    {
        ...
        private void LoadImage (ReportComponentBase parent, XmlNode xmlObject, bool useAbsTop)
        {
            ...
            else
            {
                ...
                {
                    var rect = ParseRectangleF (xmlObject["ClientRectangle"].InnerText);
                ...
                else if (xmlObject["ImageBytes"] != null)
                {
                    ...
                    var arrayimg = Convert.FromBase64String (xmlObject["ImageBytes"].InnerText);
                ...
                {
                    PictureObject.SizeMode = UnitsConverter.ConvertBool (xmlObject["AspectRatio"].InnerText)
        ...
        {
            var checkBox = ComponentsFactory.CreateCheckBoxObject (xmlObject["Name"].InnerText, band);
            LoadReportComponentBase (checkBox, xmlObject);

            if (band is TableCell)
            {
                checkBox.Bounds = new RectangleF (1, 1, Math.Min (checkBox.Width, checkBox.Height) - 1,
                    Math.Min (checkBox.Width, checkBox.Height) - 1);
            }
            else if (useAbsTop)
            {
            ...
            {
                checkBox.Checked =
                    UnitsConverter.ConvertBool (xmlObject["Checked"].InnerText.Replace ("{", "").Replace ("}", ""));
            ...
            {
                checkBox.CheckColor =
                    UnitsConverter.ConvertBrushToColor (UnitsConverter.ConvertBrush (xmlObject["TextBrush"].InnerText));
            }
        ...
        private void LoadShape (BandBase band, XmlNode xmlObject, bool useAbsTop)
        {
            switch (RemovePrefix (xmlObject["ShapeType"].Attributes["type"].Value))
            {
                ...
                {
                    var lineObject = ComponentsFactory.CreateLineObject (xmlObject["Name"].InnerText, band);
                    LoadReportComponentBase (lineObject, xmlObject);
                    lineObject.Diagonal = true;

                    var rectangle = ParseRectangleF (xmlObject["ClientRectangle"].InnerText);
                    lineObject.Bounds = new RectangleF (rectangle.X, rectangle.Y + rectangle.Height, rectangle.Width,
                        -rectangle.Height);
        ...
        {
            var zipCode = ComponentsFactory.CreateZipCodeObject (xmlObject["Name"].InnerText, band);
        ...
        {
            var matrix = ComponentsFactory.CreateMatrixObject (xmlObject["Name"].InnerText, band);
            ...
            foreach (XmlNode node in xmlObject["Components"])
            {
                switch (RemovePrefix (node.Attributes["type"].Value))
                {
                    case "CrossRow":
                        var row =
                            new MatrixHeaderDescriptor (node["Value"].InnerText.Replace ("}", "").Replace ("{", ""));
                        matrix.Data.Rows.Add (row);
                        matrix.BuildTemplate();

                        var rectCellOfRow = ParseRectangleF (node["ClientRectangle"].InnerText);
                    ...
                    case "CrossColumn":
                        ...
                        var rectCellOfColumn = ParseRectangleF (node["ClientRectangle"].InnerText);
        ...
        {
            var table = ComponentsFactory.CreateTableObject (xmlObject["Name"].InnerText, band);
            LoadReportComponentBase (table, xmlObject);

            var fixRows = 0;
            DataBand nonHeaderPartOfTab = null;

            var rectangle = table.Bounds;
            if (useAbsTop)
            {
            ...
            var columnCount = 5; //default value
            ...
            foreach (XmlNode cell in xmlObject["Components"])
            {
                if (index % columnCount == 0)
                {
                    ...
                    {
                        var subreport =
                            ComponentsFactory.CreateSubreportObject ("SubReport_" + xmlObject["Name"].InnerText, band);
        ...
        {
            var barcodeObject = ComponentsFactory.CreateBarcodeObject (xmlObject["Name"].InnerText, band);
        ...
        private void LoadTextObjBase (TextObject textObject, XmlNode xmlObject)
        {
            ...
            {
                textObject.TextRenderType = UnitsConverter.ConvertBool (xmlObject["AllowHtmlTags"].InnerText)
                    ? TextRenderType.HtmlTags
            ...
            foreach (XmlNode xmlHighlight in xmlObject["Conditions"])
            {
                try
                {
                    ...
                    var highlight = new HighlightCondition
                    {
                        Expression = UnitsConverter.ConvertRTF (parametrs[0]).Replace ("value", "Value")
                            .Replace ("{", "").Replace ("}", ""),
                        TextFill = new SolidFill (UnitsConverter.ConvertColor (UnitsConverter.ConvertRTF (parametrs[1]))),
                        Fill = new SolidFill (UnitsConverter.ConvertColor (UnitsConverter.ConvertRTF (parametrs[2]))),
                        Font = ParseFont (UnitsConverter.ConvertRTF (parametrs[3])),
                        Border =
                        {
                            Lines = UnitsConverter.ConvertBorderSides (UnitsConverter.ConvertRTF (parametrs[8]))
                        },
                        ApplyFont = true,
                        ApplyFill = true,
                        ApplyTextFill = true,
        ...
        {
            var textObject = ComponentsFactory.CreateTextObject (xmlObject["Name"].InnerText, band);
        ...
        {
            var CellularTextObject =
                ComponentsFactory.CreateCellularTextObject (xmlObject["Name"].InnerText, band);
        ...
        {
            var shapeObject = ComponentsFactory.CreateShapeObject (xmlObject["Name"].InnerText, band);
        ...
        {
            var subreportObject =
                ComponentsFactory.CreateSubreportObject (xmlObject["Name"].InnerText, band);
            var page = ComponentsFactory.CreateReportPage (Report);
        ...
        private void LoadPage (XmlNode node)
        {
            ...
            {
                page.PaperWidth = UnitsConverter.ConvertFloat (node["PageWidth"].InnerText) *
                    UnitsConverter.GetPixelsInUnit (unitType) / Units.Millimeters;
                page.PaperHeight = UnitsConverter.ConvertFloat (node["PageHeight"].InnerText) *
                    UnitsConverter.GetPixelsInUnit (unitType) / Units.Millimeters;
            }
            ...
            {
                var pagePadding = ParsePageMargins (node["Margins"].InnerText);
            ...
            {
                page.Columns.Width = UnitsConverter.ConvertFloat (node["ColumnWidth"].InnerText) *
                    UnitsConverter.GetPixelsInUnit (unitType) / Units.Millimeters;
            }

            if (node["ColumnGaps"] != null)
            {
                page.Columns.Width += UnitsConverter.ConvertFloat (node["ColumnGaps"].InnerText) *
                    UnitsConverter.GetPixelsInUnit (unitType) / Units.Millimeters;
            }
        ...
        {
            foreach (var item in SortNodeByTop (node))
            {
            ...
            foreach (var item in SortNodeByTop (node))
            {
                ...
                switch (RemovePrefix (item.Attributes["type"].Value))
                {
                    case "DataBand":
                    {
                        ...
                        {
                            (band as DataBand).Columns.Width =
                                UnitsConverter.ConvertFloat (item["ColumnWidth"].InnerText) *
                                UnitsConverter.GetPixelsInUnit (unitType);
                        }

                        if (item["ColumnGaps"] != null)
                        {
                            (band as DataBand).Columns.Width +=
                                UnitsConverter.ConvertFloat (item["ColumnGaps"].InnerText) *
                                UnitsConverter.GetPixelsInUnit (unitType);
                        }
                    ...
                    case "HierarchicalBand":
                    {
                        ...
                        if (item["DataSourceName"] != null)
                        {
                            ...
                            {
                                (band as DataBand).IdColumn = item["DataSourceName"].InnerText + "." +
                                                              item["KeyDataColumn"].InnerText;
                            }

                            if (item["MasterKeyDataColumn"] != null)
                            {
                                (band as DataBand).ParentIdColumn = item["DataSourceName"].InnerText + "." +
                                                                    item["MasterKeyDataColumn"].InnerText;
                            }
                        }
                    ...
                    default:
                    {
                        ...
                        {
                            var dataBand = ComponentsFactory.CreateDataBand (page);
                            var rect = ParseRectangleF (item["ClientRectangle"].InnerText);
                            dataBand.Bounds = rect;
                            foreach (Base obj in page.AllObjects)
                            {
                                if (obj is DataBand band1 && obj != dataBand &&
                                    band1.Bounds.Top + band1.Bounds.Height >= rect.Top)
                                {
        ...
        private void LoadChildBands (XmlNode node, ReportPage page)
        {
            foreach (var item in SortNodeByTop (node))
            {
                switch (RemovePrefix (item.Attributes["type"].Value))
                {
                    case "HeaderBand":
                    {
                        var parent = GetParentDataBand (ParseRectangleF (item["ClientRectangle"].InnerText), page,
                            true);
                        BandBase newBand;
                        if (parent is DataBand band)
                        {
                            newBand = ComponentsFactory.CreateDataHeaderBand (band);
                        }
                        else
                        {
                    ...
                    case "FooterBand":
                    {
                        ...
                        if (parent is DataBand band)
                        {
                            newBand = ComponentsFactory.CreateDataFooterBand (band);
                        }
                        else
                        {
                            newBand = ComponentsFactory.CreateChildBand (parent);
                    ...
                    {
                        BandBase band = ComponentsFactory.CreateGroupFooterBand (
                            GetParentGroupHeaderBand (ParseRectangleF (item["ClientRectangle"].InnerText), page));
                        LoadBandBase (band, item);
                        break;
                    }
                    case "ChildBand":
                    {
                        var childBand = ComponentsFactory.CreateChildBand (
                            GetParentBandBase (ParseRectangleF (item["ClientRectangle"].InnerText), page));
        ...
        private void LoadReportComponentBase (ReportComponentBase obj, XmlNode node)
        {
            ...
            {
                var rect = ParseRectangleF (node["ClientRectangle"].InnerText);
                rect.X += leftOffset;
                obj.Bounds = rect;
            }
            ...
            if (obj is BreakableComponent component)
            {
                if (node["CanBreak"] != null)
                {
                    component.CanBreak = UnitsConverter.ConvertBool (node["CanBreak"].InnerText);
                }
            }
        }
        ...
        private BandBase GetParentBandBase (RectangleF rect, ReportPage page)
        {
            ...
            {
                if (obj is BandBase @base && @base.Bottom - rect.Top > 0 && @base.Bottom - rect.Top < offset)
                {
                    result = @base;
                    offset = @base.Bottom - rect.Top;
                }
            }
        ...
        private GroupHeaderBand GetParentGroupHeaderBand (RectangleF rect, ReportPage page)
        {
            ...
            foreach (ReportComponentBase obj in page.AllObjects)
            {
                if (obj is GroupHeaderBand band && rect.Top - band.Bottom > 0 && rect.Top - band.Bottom < offset)
                {
                    result = band;
                    offset = band.Bottom - rect.Top;
                }
            }
        ...
        private BandBase GetParentDataBand (RectangleF rect, ReportPage page, bool isTopBand)
        {
            ...
            foreach (ReportComponentBase obj in page.AllObjects)
            {
                if (isTopBand)
                {
                    if (obj is DataBand band && band.Top - rect.Bottom >= 0 && band.Top - rect.Bottom < offset)
                    {
                        if (band.Header != null)
                        {
                            result = band.Header;
                        ...
                        offset = band.Top - rect.Bottom;
                    }
                }
                else
                {
                    if (obj is DataBand band && rect.Top - band.Bottom >= 0 && rect.Top - band.Bottom < offset)
                    {
                        if (band.Footer != null)
                        {
                            result = band.Footer;
        ...
        private void ParseTextOptions (TextObject textObject, string data)
        {
            ...
            if (props[5].Contains ("="))
            {
                int.TryParse (props[5].Split ('=')[1], System.Globalization.NumberStyles.Float,
                    System.Globalization.NumberFormatInfo.InvariantInfo, out angle);
            }
        ...
        {
            var marg = new float[4];
            var index = 0;
            foreach (var item in rect.Split (','))
            {
                float.TryParse (item, System.Globalization.NumberStyles.Float,
                    System.Globalization.NumberFormatInfo.InvariantInfo, out marg[index]);
        ...
        private void LoadReferencedAssemblies (XmlNode references)
        {
            foreach (XmlNode reference in references.ChildNodes)
            {
                ...
                var dllName = reference.InnerText;
                if (string.Compare (reference.InnerText.Substring (reference.InnerText.Length - 4), ".dll", true) ==
                    0 ||
        ...
        private void LoadStyles()
        {
            ...
            {
                var newStyle = ComponentsFactory.CreateStyle (style["Name"].InnerText, Report);
        ...
        private void LoadDataSource()
        {
            foreach (XmlNode db in reportNode["Dictionary"]["Databases"])
            {
                switch (RemovePrefix (db.Attributes["type"].Value))
                {
                    case "XmlDatabase":
                    {
                        var connection = new XmlDataConnection();
                        var stringBuilder = new XmlConnectionStringBuilder();
                    ...
                    case "CsvDatabase":
                    {
                        ...
                        var connection = new CsvDataConnection();
                        var stringBuilder = new CsvConnectionStringBuilder
                        {
                            CsvFile = db["PathData"].InnerText,
                            Codepage = UnitsConverter.ConvertInt (db["CodePage"].InnerText),
                            Separator = db["CodePage"].InnerText
                        };
        ...
        private void UpdateSubReportPage()
        {
            foreach (Base obj in Report.AllObjects)
            {
                if (obj is SubreportObject subreportObject)
                {
                    ...
                    {
                        if (page.Tag == subreportObject.Tag)
                        {
                            subreportObject.ReportPage = page;
                        }
                    }
