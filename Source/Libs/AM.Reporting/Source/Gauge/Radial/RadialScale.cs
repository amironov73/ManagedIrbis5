...
namespace AM.Reporting.Gauge.Radial
{
    ...
    public class RadialScale : GaugeScale
    {
        ...
        [Browsable (false)]
        internal PointF AvrTick { get; private set; }

        [Browsable (false)]
        internal double StepValue { get; set; }

        [Browsable (false)]
        internal double AverageValue { get; set; }

        internal float MajorStep { get; set; }
            ...
            VertAlign vAlign)
        {
            ...
            var strSize = RadialUtils.GetStringSize (e, Parent, Font, text);
            float dx = 0;
            float dy = 0;
        ...
        {
            var dx = Math.Abs (tick[1].X - tick[0].X);
            var dy = Math.Abs (tick[1].Y - tick[0].Y);
            var absA = (float)Math.Sqrt (Math.Pow (dx, 2) + Math.Pow (dy, 2)); //vectors length
            var sinAlpha = dy / absA;
            var cosAlpha = dx / absA;
            var absA1 = absA + txtOffset;
            var dx1 = Math.Abs (absA1 * cosAlpha);
            var dy1 = Math.Abs (absA1 * sinAlpha);
            float pointX;
            float pointY;
            if (negativ)
            {
        ...
        private void DrawMajorTicks (FRPaintEventArgs e)
        {
            ...
            AverageValue = Parent.Minimum + (Parent.Maximum - Parent.Minimum) / 2;

            var isRightPart = true;
            ...
            var maxTxt = RadialUtils.GetStringSize (e, Parent, Font, Parent.Maximum.ToString());
            var minTxt = RadialUtils.GetStringSize (e, Parent, Font, Parent.Minimum.ToString());
            var maxTxtOffset = maxTxt.Height > maxTxt.Width ? maxTxt.Height : maxTxt.Width;
            var minTxtOffset = minTxt.Height > minTxt.Width ? minTxt.Height : minTxt.Width;
            majorTicksOffset = maxTxtOffset > minTxtOffset ? maxTxtOffset : minTxtOffset;

            var tick0 = new PointF[2];
            ...
            double angle = 0;
            var horAlign = HorAlign.Middle;
            var vertAlign = VertAlign.Bottom;
            var startValue = AverageValue;
            ...
            for (var i = 0; i < sideTicksCount; i++)
            {
                ...
                if (drawLeft)
                {
                    ...
                    if (i == sideTicksCount / 2)
                    {
                        if (RadialUtils.IsSemicircle (Parent) || RadialUtils.IsQuadrant (Parent))
                        {
                            if ((RadialUtils.IsTop (Parent) || RadialUtils.IsBottom (Parent)) &&
                                RadialUtils.IsSemicircle (Parent))
                            {
                                horAlign = HorAlign.Right;
                                vertAlign = VertAlign.Middle;
                            }
        ...
        private void DrawMinorTicks (FRPaintEventArgs e)
        {
            ...
            var center = new PointF (left + width / 2, top + height / 2);
            ...
            for (var i = 0; i < MajorTicks.Count / 2 * (MinorTicks.Count + 1); i++)
            {
