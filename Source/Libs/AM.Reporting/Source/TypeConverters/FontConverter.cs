...
namespace AM.Reporting.TypeConverters
{
    public class FontConverter : TypeConverter
    {
        ...
        [Obsolete] public static FontConverter Instance = new FontConverter();
            ...
            Type destinationType)
        {
            if (value is Font font)
            {
                ...
                {
                    var met = typeof (Font).GetConstructor (new Type[]
                        { typeof (string), typeof (float), typeof (FontStyle), typeof (GraphicsUnit) });
        ...
        public override object ConvertFrom (ITypeDescriptorContext context, CultureInfo culture, object value)
        {
            ...
            var separator = culture.TextInfo.ListSeparator[0]; // For vi-VN: ','
            var fontName = font; // start with the assumption that only the font name was provided.
            string style = null;
            string sizeStr = null;
            var fontSize = 8.25f;
            ...
            var nameIndex = font.IndexOf (separator);
            ...
            if (nameIndex < font.Length - 1)
            {
                ...
                var styleIndex = culture.CompareInfo.IndexOf (font, StylePrefix, CompareOptions.IgnoreCase);
                ...
                if (unitTokensSize != null)
                {
                    try
                    {
                        fontSize = (float)TypeDescriptor.GetConverter (typeof (float))
                            .ConvertFromString (context, culture, unitTokensSize);
                    }
                    catch
                    {
                ...
                if (style != null)
                {
                    ...
                    for (var tokenCount = 0; tokenCount < styleTokens.Length; tokenCount++)
                    {
                        var styleText = styleTokens[tokenCount];
                        styleText = styleText.Trim();

                        fontStyle |= (FontStyle)Enum.Parse (typeof (FontStyle), styleText, true);

                        // Enum.IsDefined doesn't do what we want on flags enums...
                        var validBits = FontStyle.Regular | FontStyle.Bold | FontStyle.Italic |
            ...
            var result = new Font (fontName, fontSize, fontStyle, units);
        ...
        private void ParseSizeTokens (string text, char separator, ref string size, ref string units)
        {
            ...
            if (length > 0)
            {
                ...
                var trimChars = new char[] { separator, ' ' };
        ...
        private GraphicsUnit ParseGraphicsUnits (string units)
        {
            ...
            {
                throw new ArgumentException ("Invalid font units: " + units);
            }
        }
        ...
        public override object CreateInstance (ITypeDescriptorContext context, IDictionary propertyValues)
        {
            ...
            else
            {
                ...
                foreach (var font in installedFontList)
                {
                ...
                if (fontFamily == null)
                {
                    ...
                    foreach (var font in privateFontList)
                    {
                        if (name.Equals (font.Name, StringComparison.OrdinalIgnoreCase))
                        {
                            fontFamily = font;
        ...
        public sealed class FontNameConverter : TypeConverter, IDisposable
        {
            ...
            {
                var values = new string[_fonts.Length];
        ...
        public class FontUnitConverter : EnumConverter
        {
            ...
            public override StandardValuesCollection GetStandardValues (ITypeDescriptorContext context)
            {
                ...
                if (Values == null)
                {
                    ...
                    var filteredValues = new ArrayList (Values);
