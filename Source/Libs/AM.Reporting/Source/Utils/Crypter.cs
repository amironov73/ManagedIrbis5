...
namespace AM.Reporting.Utils
{
    ...
    public static class Crypter
    {
      ...
      using (PasswordDeriveBytes pdb = new PasswordDeriveBytes(password, Encoding.UTF8.GetBytes("Salt")))
        ...
        public static string EncryptString (string data, string password)
        {
            ...
            {
                using (var cryptedStream = Encrypt (stream, password))
                {
        ...
        public static string DecryptString (string data, string password)
        {
            ...
            using (var stream = Converter.FromString (typeof (Stream), data) as Stream)
            {
                using (var decryptedStream = Decrypt (stream, password))
                {
                    ...
                    {
                        var n = decryptedStream.Read (bytes, bytesRead, bytes.Length - bytesRead);
                        if (n == 0)
                        {
                            break;
                        }
        ...
        {
            var hash = new Murmur3().ComputeHash (input);
    ...
    public class Murmur3
    {
        ...
        /// READ_SIZE
        /// </summary>
        [CLSCompliantAttribute (false)] public static ulong READ_SIZE = 16;
