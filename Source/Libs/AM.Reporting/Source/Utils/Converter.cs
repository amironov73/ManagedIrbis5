    ...
    public static class Converter
    {
        ...
        public static string ToString (object value)
        {
            ...
            if (value is Image image)
            {
                ...
                {
                    ImageHelper.Save (image, stream);
                    return Convert.ToBase64String (stream.ToArray());
                }
            }

            if (value is Stream stream1)
            {
            ...
            {
                var shortTypeName = type.Assembly.FullName.StartsWith ("mscorlib") ||
                                    type.Assembly == typeof (Converter).Assembly ||
                                    type.Assembly.FullName.StartsWith ("System.Private.CoreLib");
...
#endif
            if (value is System.Drawing.Imaging.ImageFormat imageFormat)
            {
            ...
            while (i < s.Length)
            {
                if (s[i] == '&')
                {
                    if (i + 3 < s.Length && s[i + 1] == '#')
                    {
                        ...
                        if (s[i + 2] == 'x')
                        {
                            var hexChar = (char)int.Parse (s.Substring (i + 3, j - i - 3),
        ...
        {
            var currentSeparator = CultureInfo.CurrentCulture.NumberFormat.CurrencyDecimalSeparator;
            ...
            bool removeNonDigit)
        {
            ...
            var separatorPos = value.IndexOf (separator);
            if (separatorPos == -1)
            {
        ...
        {
            var fastString = new StringBuilder (2);
        ...
        public static bool FromHtmlEntities (string text, ref int position, StringBuilder result)
        {
            ...
            if (text[position + 1] == '#')
            {
                ...
                {
                    //hex
                    var fastString = new StringBuilder (end - position);
                    for (var i = position + 3; i < end; i++)
                    {
                    ...
                    try
                    {
                        var value = Convert.ToInt32 (fastString.ToString(), 16);
                ...
                {
                    //dec
                    var fastString = new FastString (end - position);
                    for (var i = position + 2; i < end; i++)
                    {
                    ...
                    try
                    {
                        var value = Convert.ToInt32 (fastString.ToString(), 10);
                        if (value > char.MaxValue)
                        {
                            result.Append (char.ConvertFromUtf32 (value));
                        }
            ...
            var code = GetCode (text.Substring (position, end - position + 1));
            if (code == 0)
            {
                return false;
            }
