...
namespace AM.Reporting.Barcode.Aztec
{
    ...
    internal sealed class HighLevelEncoder
    {
        ...
        static HighLevelEncoder()
        {
            ...
            foreach (var table in SHIFT_TABLE)
            {
        ...
        public BitArray encode()
        {
            ...
            for (var index = 0; index < text.Length; index++)
            {
                ...
                var nextChar = (index + 1 < text.Length) ? (int)text[index + 1] : 0;
                switch (text[index])
                {
                    case (byte)'\r':
                        pairCode = nextChar == '\n' ? 2 : 0;
        ...
        {
            var ch = (char)(text[index] & 0xFF);
            var charInCurrentTable = CHAR_MAP[state.Mode][ch] > 0;
            ...
            for (var mode = 0; mode <= MODE_PUNCT; mode++)
            {
                ...
                if (charInMode > 0)
                {
                    ...
                    if (!charInCurrentTable || mode == state.Mode || mode == MODE_DIGIT)
                    {
                        ...
                        var latch_state = stateNoBinary.latchAndAppend (mode, charInMode);
                    ...
                    if (!charInCurrentTable && SHIFT_TABLE[state.Mode][mode] >= 0)
                    {
                        ...
                        var shift_state = stateNoBinary.shiftAndAppend (mode, charInMode);
