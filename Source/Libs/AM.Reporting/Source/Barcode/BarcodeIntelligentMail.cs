...
namespace AM.Reporting.Barcode
{
    ...
    public class BarcodeIntelligentMail : LinearBarcodeBase
    {
        ...
        {
            base.Assign (source);
            var src = source as BarcodeIntelligentMail;
        ...
        int[] table2Of13;
        int[] table5Of13;
        decimal[][] codewordArray;

        int[] barTopCharIndexArray = new int[]
        {
            4, 0, 2, 6, 3, 5, 1, 9, 8, 7, 1, 2, 0, 6, 4, 8, 2, 9, 5, 3, 0, 1, 3, 7, 4, 6, 8, 9, 2, 0, 5, 1, 9, 4, 3, 8,
            6, 7, 1, 2, 4, 3, 9, 5, 7, 8, 3, 0, 2, 1, 4, 0, 9, 1, 7, 0, 2, 4, 6, 3, 7, 1, 9, 5, 8
        };

        int[] barBottomCharIndexArray = new int[]
        {
            7, 1, 9, 5, 8, 0, 2, 4, 6, 3, 5, 8, 9, 7, 3, 0, 6, 1, 7, 4, 6, 8, 9, 2, 5, 1, 7, 5, 4, 3, 8, 7, 6, 0, 2, 5,
            4, 9, 3, 0, 1, 6, 8, 2, 0, 4, 5, 9, 6, 7, 5, 2, 6, 3, 8, 5, 1, 9, 8, 7, 4, 0, 2, 6, 3
        };

        int[] barTopCharShiftArray = new int[]
        {
        ...
        int[] barBottomCharShiftArray = new int[]
        {
        ...
        string Bars (string source)
        {
            ...
            {
                l = long.Parse (zip, CultureInfo.InvariantCulture) + ((zip.Length == 5)
                    ? 1
                    : ((zip.Length == 9) ? 100001 : (zip.Length == 11 ? 1000100001 : 0)));
            }
            ...
            for (short i = 0; i <= 9; i++)
            {
                ...
                ad[i][0] = 8192;
                ad[i][1] = (codewordArray[i][1] >= (decimal)entries2Of13)
                    ? ad[i][1] = table2Of13[(int)(codewordArray[i][1] - entries2Of13)]
                    : ad[i][1] = table5Of13[(int)codewordArray[i][1]];
            }
        ...
        bool OneCodeMathAdd (ref int[] bytearray, int i, int j)
        {
            ...
            var x = (bytearray[i - 1] | (bytearray[i - 2] << 8)) + j;
            var l = x | 65535;
            var k = i - 3;
            bytearray[i - 1] = x & 255;
        ...
        bool OneCodeMathDivide (string v)
        {
            ...
            {
                var r = string.Empty;
                var divider = (int)codewordArray[k][0];
                var copy = n;
                var left = "0";
                var l = copy.Length;
                for (short i = 1; i <= l; i++)
                {
                    var divident = int.Parse (copy.Substring (0, i), CultureInfo.InvariantCulture);
                ...
                if (k == 1)
                {
                    codewordArray[0][1] = int.Parse (r, CultureInfo.InvariantCulture);
                }
            }
