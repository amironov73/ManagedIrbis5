...
namespace AM.Reporting.Barcode
{
    ...
    public sealed class BarcodeDatamatrix : Barcode2DBase
    {
            ...
            int dataLength)
        {
            ...
            {
                var i = x12.IndexOf ((char)text[ptrIn + textOffset]);
                if (i >= 0)
                {
                    x[ptrIn] = (byte)i;
                    ++count;
            ...
            int dataLength)
        {
            ...
            var dataSize = int.MaxValue;
            for (var i = 0; i < dmSizes.Length; ++i)
            {
        ...
        private static int C40OrTextEncodation (byte[] text, int textOffset, int textLength, byte[] data,
            int dataOffset, int dataLength, bool c40)
        {
        ...
        private void Generate (string text)
        {
            ...
            var t = System.Text.Encoding.GetEncoding (CodePage).GetBytes (text);
        ...
        {
            base.Assign (source);
            var src = source as BarcodeDatamatrix;
        ...
        internal override void Serialize (FRWriter writer, string prefix, BarcodeBase diff)
        {
            ...
            var c = diff as BarcodeDatamatrix;
        ...
        internal override void Initialize (string text, bool showText, int angle, float zoom)
        {
            ...
            {
                var dmParams = dmSizes[(int)SymbolSize - 1];
                ...
                for (i = 0; i < nd; i++)
                {
                    ...
                    for (j = 0; j < nc; j++)
                    {
                        ncout[j] = (byte)(ncout[j + 1] ^
                                          (k == 0 ? (byte)0 : (byte)alog[(log[k] + log[c[nc - j - 1]]) % (255)]));
                    }
                }
            }
