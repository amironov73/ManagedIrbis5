    ...
    internal sealed class Encoder
    {
        ...
        private static void initQRCode (int numInputBytes, ErrorCorrectionLevel ecLevel, Mode mode, QRCode qrCode)
        {
            ...
            for (var versionNum = 1; versionNum <= 40; versionNum++)
            {
                ...
                // getNumRSBlocks = 5
                var numRSBlocks = ecBlocks.NumBlocks;
        ...
        internal static void interleaveWithECBytes (BitVector bits, int numTotalBytes, int numDataBytes,
            int numRSBlocks, BitVector result)
        {
            ...
            var blocks =
                System.Collections.ArrayList.Synchronized (new System.Collections.ArrayList (numRSBlocks));
            ...
            for (var i = 0; i < maxNumDataBytes; ++i)
            {
                ...
                {
                    var dataBytes = ((BlockPair)blocks[j]).DataBytes;
                    if (i < dataBytes.size())
                    {
            ...
            for (var i = 0; i < maxNumEcBytes; ++i)
            {
                for (var j = 0; j < blocks.Count; ++j)
                {
                    var ecBytes = ((BlockPair)blocks[j]).ErrorCorrectionBytes;
                    if (i < ecBytes.size())
                    {
