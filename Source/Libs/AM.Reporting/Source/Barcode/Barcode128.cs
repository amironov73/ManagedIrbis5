...
#pragma warning restore FR0006 // Field name of struct must be longer than 2 characters.
        ...
        {
            for (var i = 0; i < tabelle_128.Length; i++)
            {
        ...
        private string GetNextPortion (string code, ref int index, ref Encoding encoding)
        {
            ...
            var aIndex = FindCodeA (code[index]);
            var bIndex = FindCodeB (code[index]);
            var firstCharEncoding = Encoding.A;
            ...
            while (index + numChars < code.Length)
            {
                ...
                var nextCharEncoding = Encoding.A;
                if (aIndex == -1 && bIndex != -1)
                {
            ...
            var prefix = firstCharEncoding == Encoding.A ? "&A;" : "&B;";
        ...
        private string StripControlCodes (string code, bool stripFNCodes)
        {
            ...
            {
                var nextChar = GetNextChar (code, ref index, Encoding.None);
        ...
        internal override string GetPattern()
        {
            ...
            var encoding = Encoding.None;
            var index = 0;
            var nextChar = GetNextChar (code, ref index, encoding);
            var checksum = 0;
            var startCode = "";
            ...
            var result = startCode; // Startcode
            var codeword_pos = 1;
        ...
        internal override void Serialize (FRWriter writer, string prefix, BarcodeBase diff)
        {
            ...
            if (diff is not Barcode128 c || AutoEncode != c.AutoEncode)
            {
