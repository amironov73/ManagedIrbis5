...
internal static class HunspellTextFunctions
{
    ...
    public static bool MyIsAlpha (char ch)
    {
    ...
    {
        return (c < 127 || textInfo.ToUpper (c) != c) && char.IsLower (c);
    }
    ...
    public static string WithoutChars (this string @this, CharacterSet chars)
    {
        ...
        if (@this.Length == 0 || chars.IsEmpty) return @this;
        ...
        for (; index < thisSpan.Length; index++)
        {
            ...
            if (!chars.Contains (c)) builder.Append (c);
        }
    ...
    public static string MakeInitCap (string s, TextInfo textInfo)
    {
        ...
        if (expectedFirstLetter == actualFirstLetter) return s;

        var builder = StringBuilderPool.Get (s.Length);
        builder.Append (expectedFirstLetter);
        if (s.Length > 1) builder.Append (s, 1, s.Length - 1);
        return StringBuilderPool.GetStringAndReturn (builder);
    }
    ...
    public static string MakeInitCap (ReadOnlySpan<char> s, TextInfo textInfo)
    {
        ...
        var expectedFirstLetter = textInfo.ToUpper (actualFirstLetter);
        if (expectedFirstLetter == actualFirstLetter) return s.ToString();

        var builder = StringBuilderPool.Get (s.Length);
        builder.Append (expectedFirstLetter);
        if (s.Length > 1) builder.Append (s.Slice (1));
        return StringBuilderPool.GetStringAndReturn (builder);
    }
    ...
    public static string MakeTitleCase (string s, CultureInfo cultureInfo)
    {
        ...
        using (var mo = MemoryPool<char>.Shared.Rent (s.Length))
        {
            ...
            if (sSpan.Length > 1) sSpan.Slice (1).ToLower (buffer.Slice (1), cultureInfo);
    ...
    {
        if (Encoding.UTF8.Equals (encoding)) return decoded;
        ...
        unsafe
        {
            fixed (char* decodedPointer = &MemoryMarshal.GetReference (decoded))
            {
                ...
                {
                    encodedBytesCount = encoding.GetBytes (decodedPointer, decoded.Length, encodedBytesPointer,
                        encodedBytes.Length);
                }
            }
        }
    ...
    {
        return GetCapitalizationType (word.AsSpan(), textInfo);
    }

    public static CapitalizationType GetCapitalizationType (ReadOnlySpan<char> word, TextInfo textInfo)
    {
        if (word.IsEmpty) return CapitalizationType.None;
        ...
        for (var i = 0; i < word.Length; i++)
        {
            ...
            {
                hasLower = true;
                if (hasFoundMoreCaps) break;
            }
        }

        if (firstIsUpper)
        {
            if (!hasFoundMoreCaps) return CapitalizationType.Init;
            if (!hasLower) return CapitalizationType.All;

            return CapitalizationType.HuhInit;
        }
        else
        {
            if (!hasFoundMoreCaps) return CapitalizationType.None;
            if (!hasLower) return CapitalizationType.All;
