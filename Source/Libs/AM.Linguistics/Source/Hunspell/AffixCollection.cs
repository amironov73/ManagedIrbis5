    ...
    where TEntry : AffixEntry
{
    ...
    internal static TResult Create<TResult> (List<AffixEntryGroup<TEntry>.Builder> builders,
...
public sealed class SuffixCollection : AffixCollection<SuffixEntry>
{
    ...
    {
        if (builders == null || builders.Count == 0) return Empty;
    ...
    {
        if (string.IsNullOrEmpty (word)) return new List<Affix<SuffixEntry>> (0);
...
public sealed class PrefixCollection : AffixCollection<PrefixEntry>
{
    ...
    {
        if (string.IsNullOrEmpty (word)) return new List<Affix<PrefixEntry>> (0);
