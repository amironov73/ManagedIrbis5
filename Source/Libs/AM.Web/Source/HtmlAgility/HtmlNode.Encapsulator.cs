...
public partial class HtmlNode
{
    ...
    /// <exception cref="FormatException">Why it's thrown.</exception>
    /// <exception cref="Exception">Why it's thrown.</exception>
    public object GetEncapsulatedData
        (
            Type targetType,
        ...
        )
    {
        ...
        var source = htmlDocument ?? OwnerDocument;
        object targetObject;
        ...
        if (targetType.IsDefinedAttribute (typeof (HasXPathAttribute))) // Object has xpath attribute (Defined HasXPath)
        {
            ...
            else
            {
                ...
                foreach (PropertyInfo propertyInfo in validProperties)
                {
                    ...
                    var xPathAttribute =
                        (propertyInfo.GetCustomAttributes (typeof (XPathAttribute), false) as IList)[0] as
                        XPathAttribute;
                            ...
                            else
                            {
                                ...
                                if (result == null)
                                {
                                    throw new NodeAttributeNotFoundException ("Can not find " +
                                                                              ...
                                                                              " related to " + propertyInfo.PropertyType.FullName + " " +
                                                                              propertyInfo.Name);
                                }
                            ...
                            if (nodeCollection == null! || nodeCollection.Count == 0)
                            {
                                ...
                                else
                                {
                                    throw new NodeNotFoundException (
                                        "Cannot find node with giving XPath to bind to " +
                                        propertyInfo.PropertyType.FullName + " " + propertyInfo.Name);
                                }
                            }
                            else
                                ...
                                else // T is value-type or .Net class or user-defined class ( without xpath )
                                {
                                    ...
                                    else // It target attribute
                                    {
                                        foreach (var node in nodeCollection)
                                        {
                                            ...
                                            {
                                                throw new NodeAttributeNotFoundException ("Can not find " +
                                                    xPathAttribute.AttributeName + " Attribute in " + node.Name +
                                                    " related to " +
                                                    propertyInfo.PropertyType.FullName + " " + propertyInfo.Name);
                                            }
                                            ...
                                            try
                                            {
                                                ...
                                                (FormatException) // if it can not cast result(string) to type of property.
                                            {
                                                throw new FormatException ("Can not convert Invalid string to " +
                                                                           T_Types[0].FullName + " " +
                                                                           propertyInfo.Name);
...
internal static class Tools
{
    ...
    internal static bool IsDefinedAttribute (this Type type, Type attributeType)
    {
        ...
        throw new Exception ("Can't Target any platform when checking " + type.FullName + " is a " +
    ...
    internal static IList CreateIListOfType
        (
            this Type type
        )
    {
        ...
        return (Activator.CreateInstance (constructedListType) as IList).ThrowIfNull();
    }
