...
public class Common
{
    ...
    internal static void ThrowIOError (int errorCode, string maybeFullPath)
    {
        ...
        switch (errorCode)
        {
            ...
            case NativeMethods.ERROR_ALREADY_EXISTS:
                ...
                throw new IOException (string.Format ("File {0}", str),
            ...
            case NativeMethods.ERROR_INVALID_PARAMETER:
                throw new IOException (NativeMethods.GetMessage (errorCode),
                    NativeMethods.MakeHRFromErrorCode (errorCode));

            case NativeMethods.ERROR_SHARING_VIOLATION:
                if (str.Length == 0)
                {
                    throw new IOException ("Sharing violation with empty filename",
                        NativeMethods.MakeHRFromErrorCode (errorCode));
                }
                else
                {
                    throw new IOException (string.Format ("Sharing violation: {0}", str),
                        NativeMethods.MakeHRFromErrorCode (errorCode));
                }
            ...
            case NativeMethods.ERROR_FILE_EXISTS:
                ...
                throw new IOException (string.Format ("File exists {0}", str),
                    NativeMethods.MakeHRFromErrorCode (errorCode));
    ...
    internal static void SetAccessControlExtracted (FileSystemSecurity security, string name)
    {
        ...
        if ((includeSections & AccessControlSections.Audit) != AccessControlSections.None)
        {
            ...
            {
                var isSystemAclPresent = (rawSecurityDescriptor.ControlFlags & ControlFlags.SystemAclPresent) !=
                                         ControlFlags.None;
                if (isSystemAclPresent && rawSecurityDescriptor.SystemAcl != null &&
                    rawSecurityDescriptor.SystemAcl.Count > 0)
                {
                ...
                securityInfo =
                    (SecurityInfos)(((rawSecurityDescriptor.ControlFlags & ControlFlags.SystemAclProtected) ==
                                     ControlFlags.None
            ...
            isDiscretionaryAclPresent)
        {
            ...
            if (rawSecurityDescriptor != null)
            {
                ...
                securityInfo =
                    (SecurityInfos)
                    (((rawSecurityDescriptor.ControlFlags & ControlFlags.DiscretionaryAclProtected) ==
                      ControlFlags.None
        ...
        int errorNum = SetSecurityInfo (ResourceType.FileObject, name, null, securityInfo, owner, @group, sacl,
            dacl);
        if (errorNum != 0)
        {
        ...
        GenericAcl dacl)
    {
        ...
        try
        {
            ...
            if (name != null)
            {
                errorCode = (int)NativeMethods.SetSecurityInfoByName (name, (uint)type, (uint)securityInformation,
            ...
            else if (handle != null)
            {
                ...
                {
                    errorCode = (int)NativeMethods.SetSecurityInfoByHandle (handle, (uint)type,
                        (uint)securityInformation, OwnerBinary, GroupBinary, DaclBinary, SaclBinary);
                }
            }
            else
            {
