...
public static class Builtins
{
        ...
        )
    {
        ...
        if (args[0] is LambdaNode lambda)
        {
            ...
            foreach (var current in source0)
            {
                ...
                callArgs0[1] = index0;
                callArgs0[2] = source0;
                var retval = BarsikUtility.ToBoolean (lambda.Adapter (child0, callArgs0));
                if (retval)
                {
                    result0.Add (current);
                }
        ...
        )
    {
        ...
        {
            BarsikUtility.PrintObject (context.Commmon.Output, node);
        }
    ...
    {
        var count = BarsikUtility.ToInt32 (Compute (context, args, 0));
        if (count <= 0)
        {
