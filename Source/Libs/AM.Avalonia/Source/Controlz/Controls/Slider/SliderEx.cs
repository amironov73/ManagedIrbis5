    ...
    : RangeBaseEx
{
    ...
    {
        get => GetValue (IsSelectionRangeEnabledProperty);
    ...
    public static readonly StyledProperty<double> SelectionStartProperty =
        AvaloniaProperty.Register<SliderEx, double> (nameof (SelectionStart)
            , defaultValue: 0.0d, defaultBindingMode: BindingMode.TwoWay);
    ...
    private void UpdateValue (double value)
    {
        ...
        {
            SetValue (ValueProperty, Math.Max (Minimum, Math.Min (Maximum, snappedValue)));
        }
    }
    ...
    {
        SelectionRangeElement = eventArgs.NameScope.Find<AvaloniaObject> (SelectionRangeElementName);
        Track = eventArgs.NameScope.Find<Track> (TrackName);
        TopTickBar = eventArgs.NameScope.Find<TickBar> ("TopTick");
        BottomTickBar = eventArgs.NameScope.Find<TickBar> ("BottomTick");

        TrackBackground = eventArgs.NameScope.Find<Border> ("TrackBackground");
