...
namespace FluentAvalonia.UI.Controls
{
    ...
    public partial class Frame : ContentControl
    {
        ...
        public string GetNavigationState()
        {
            if (!IsNavigationStackEnabled)
            {
                throw new InvalidOperationException
                    (
                        "Cannot retrieve navigation stack when IsNavigationStackEnabled is false"
                    );
            }
            ...
            static void AppendEntry
                (
                    ...
                    PageStackEntry entry
                )
            {
                builder.Append (entry.SourcePageType.AssemblyQualifiedName);
                builder.Append ('|');
                if (entry.Parameter != null)
                {
                    builder.Append (entry.Parameter.ToString());
                }
            ...
            {
                AppendEntry (builder, CurrentEntry);
            }

            builder.AppendLine (BackStackDepth.ToString());

            for (var i = 0; i < BackStackDepth; i++)
            {
                AppendEntry (builder, BackStack[i]);
            }

            builder.AppendLine (ForwardStack.Count.ToString());
        ...
        public void SetNavigationState (string navState, bool suppressNavigate)
        {
            ...
            {
                throw new InvalidOperationException (
                    "Cannot set navigation stack when IsNavigationStackEnabled is false");
            }
            ...
            using (var reader = new StringReader (navState))
            {
                ...
                if (firstLine[0] != '|')
                {
                    ...
                    if (!suppressNavigate)
                    {
                        ...
                        page.RaiseEvent (new NavigationEventArgs (page, NavigationMode.New, null, param, pageType)
        ...
        private IControl CreatePageAndCacheIfNecessary (Type srcPageType)
        {
            ...
            for (var i = 0; i < _cache.Count; i++)
            {
                ...
                {
                    throw new Exception (
                        $"An object of type {srcPageType} has already been added to the Navigation Stack");
                }
            }
        ...
        private void SetContentAndAnimate (PageStackEntry entry)
        {
            ...
            if (_presenter != null)
            {
                ...
                Dispatcher.UIThread.Post (() => { entry.NavigationTransitionInfo.RunAnimation (_presenter); },
