...
public partial class BreadcrumbButton : HeaderedItemsControl
{
    ...
    /// <inheritdoc cref="ItemsControl.ItemsChanged"/>
    protected override void ItemsChanged
        (
            AvaloniaPropertyChangedEventArgs eventArgs
        )
    {
        base.ItemsChanged(eventArgs);

        if (contextMenu != null && eventArgs.NewValue != null)
        {
    ...
    /// <inheritdoc cref="InputElement.OnPointerLeave"/>
    ...
    /// <inheritdoc cref="InputElement.OnPointerEnter"/>
    ...
    {
        dropDownBtn = eventArgs.NameScope.Find<Control>(partDropDown);

        dropPanel = eventArgs.NameScope.Find<Control>("borderBtn");

        contextMenu = eventArgs.NameScope.Find<ContextMenu>(partMenu);
        if (contextMenu != null)
        {
    ...
    private void ContextMenu_PropertyChanged
        (
            object? sender,
            AvaloniaPropertyChangedEventArgs eventArgs
        )
    {
        if (eventArgs.Property.Name == nameof(ContextMenu.IsOpen))
        {
