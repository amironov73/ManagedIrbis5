...
public class AnimationDecorator : Decorator
{
    ...
    {
        get => GetValue (OpacityAnimationProperty);
    ...
    {
        get => GetValue (HeightAnimationProperty);
    ...
    {
        get => GetValue (AnimationOpacityProperty);
    ...
    public static readonly StyledProperty<double> AnimationOpacityProperty =
        AvaloniaProperty.Register<AnimationDecorator, double> (nameof (AnimationOpacity),
            defaultValue: 1.0);
    ...
    {
        get => GetValue (HeightOffsetProperty);
    ...
    {
        get => GetValue (AnimateOnContentHeightChangedProperty);
    ...
    public static readonly StyledProperty<bool> AnimateOnContentHeightChangedProperty =
        AvaloniaProperty.Register<AnimationDecorator, bool> (nameof (AnimateOnContentHeightChanged),
            defaultValue: true);
    ...
    private void IsExpandedChanged
        (
            AnimationDecorator expander,
            AvaloniaPropertyChangedEventArgs eventArgs
        )
    {
        var expanded = (bool) eventArgs.NewValue;
        if (expander.CanAnimate)
        {
            Task.Factory.StartNew (() =>
            {
                while (animating)
                    Thread.Sleep (10);
            }).ContinueWith (x => { expander.AnimateExpandedChanged (expanded); },
    ...
    private async void AnimateExpandedChanged
        (
            bool expanded
        )
    {
        if (Child != null)
        {
            ...
            var animation = HeightAnimation as Animation.Animation;
            if (animation == null)
            {
            ...
            var val = expanded ? 0 : -Child.DesiredSize.Height;
        ...
        )
    {
        ...
        if (double.IsInfinity (constraint.Height))
        {
            ...
            var w = IsExpanded ? Child.DesiredSize.Width : 0;
        ...
        )
    {
        ...
        var h = Math.Max (0, Child.DesiredSize.Height + YOffset);
