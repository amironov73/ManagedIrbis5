    ...
    : TemplatedControl
{
    ...
    /// <param name="e"></param>
    private void SliderAlpha_ValueChanged
        (
            object sender,
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void DefaultPicker_SelectionChanged
        (
            object? sender,
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void EllipsePointer_MouseLeftButtonUp
        (
            object? sender,
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void EllipsePointer_MouseLeftButtonDown
        (
            object? sender,
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void EllipsePointer_MouseMove
        (
            object? sender,
            PointerEventArgs e
        )
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void CanvasColor_MouseLeftButtonUp
        (
            object? sender,
            PointerReleasedEventArgs e
        )
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void CanvasColor_MouseLeftButtonDown
        (
            object? sender,
            PointerPressedEventArgs e
        )
        ...
        )
    {
        if (e.Key == Key.Enter)
        {
            try
            {
                ...
                var allTextResult = string.Empty;
        ...
        var input = e.Key.ToString().Substring (1);
        ...
        if (!(input == "#" || (input[0] >= 'A' && input[0] <= 'F') || (input[0] >= 'a' && input[0] <= 'F') ||
              (input[0] >= '0' && input[0] <= '9')))
            e.Handled = true;
        if (input.Length > 1)
            e.Handled = true;
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void txtAll_TextChanged
        (
            object sender,
            RoutedEventArgs e
        )
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void txtBlue_TextChanged
        (
            object sender,
            RoutedEventArgs e
        )
    {
        ...
        {
            if (string.IsNullOrEmpty (((TextBox)sender).Text)) return;
            var val = Convert.ToInt32 (((TextBox)sender).Text);
            if (val > 255)

                ((TextBox)sender).Text = "255";
            else
            {
                var byteValue = Convert.ToByte (((TextBox)sender).Text);
                CustomColor =
                    Color.FromArgb (
                            _customColor.A,
                            CustomColor.R,
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void txtGreen_TextChanged
        (
            object sender,
            RoutedEventArgs e
        )
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void txtRed_TextChanged
        (
            object sender,
            RoutedEventArgs e
        )
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void txtAlpha_TextChanged
        (
            object sender,
            RoutedEventArgs e
        )
    ...
    {
        for (var i = 0; i < _canvasColor.Width; i++)
        {
            var flag = false;
            for (var j = 0; j < _canvasColor.Height; j++)
            {
                try
                {
                    var Colorfromimagepoint = ColorPickerHelper.GetColorFromImage (_image, i, j);
    ...
    private void ChangeColor (PointerEventArgs e)
    {
        try
        {
            CustomColor = ColorPickerHelper.GetColorFromImage (_image,
    ...
    private void UpdatePreview()
    {
        ...
        var alphaHex = CustomColor.A.ToString ("X").PadLeft (2, '0');

        RedValue = CustomColor.R;

        _txtRed.Text = CustomColor.R.ToString();
        var redHex = CustomColor.R.ToString ("X").PadLeft (2, '0');

        GreenValue = CustomColor.G;

        _txtGreen.Text = CustomColor.G.ToString();
        var greenHex = CustomColor.G.ToString ("X").PadLeft (2, '0');

        BlueValue = CustomColor.B;

        _txtBlue.Text = CustomColor.B.ToString();
        var blueHex = CustomColor.B.ToString ("X").PadLeft (2, '0');
    ...
    private void NumericValidation (KeyEventArgs e)
    {
        ...
        try
        {
            ...
            var inputDigit = Int32.Parse (input);
        }
        catch
        {
            e.Handled = true;
    ...
    {
        var epDefaultcolor = eventArgs.NameScope.Find<Expander> (PART_DefaultColorSection);
        var epCustomcolor = eventArgs.NameScope.Find<Expander> (PART_CustomColorSection);
        ...
        _image = eventArgs.NameScope.Find<Image> (PART_Image);

        _txtAlpha = eventArgs.NameScope.Find<TextBox> (PART_NumericAlpha);
        _txtRed = eventArgs.NameScope.Find<TextBox> (PART_NumericRed);
        _txtGreen = eventArgs.NameScope.Find<TextBox> (PART_NumericGreen);
        _txtBlue = eventArgs.NameScope.Find<TextBox> (PART_NumericBlue);
        _txtAll = eventArgs.NameScope.Find<TextBox> (PART_TxtAll);
        ...
        _borderPreview = eventArgs.NameScope.Find<Border> (PART_Preview);
        _sliderAlpha = eventArgs.NameScope.Find<Slider> (PART_SliderAlpha);
        ...
        _canvasColor = eventArgs.NameScope.Find<Canvas> (PART_CanvasColor);
        _ellipsePointer = eventArgs.NameScope.Find<Ellipse> (PART_Pointer);

        _defaultPicker = eventArgs.NameScope.Find<ListBox> (PART_DefaultPicker);
