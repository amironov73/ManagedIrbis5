...
public class ClipBorder : Decorator
{
    ...
    private StreamGeometry _backgroundGeometryCache = null;
    ...
    /// <inheritdoc cref="Decorator.MeasureOverride"/>
        ...
        )
    {
        ...
        if (child != null)
        {
            ...
            desiredSize = new Size (childSize.Width + combined.Width, childSize.Height + combined.Height);
        }
        else
        {
    ...
    /// <inheritdoc cref="Decorator.ArrangeOverride"/>
        ...
        )
    {
        ...
        if ((borderBrush != null) && (!borders.IsZero()) && (bgBrush != null))
        {
            ...
            {
                context.DrawGeometry (borderBrush, null, borderGeometry);
                context.DrawGeometry (bgBrush, null, backgroundGeometry);
            }
            ...
            else if (borderBrush.IsOpaqueSolidColorBrush())
            {
                ...
                throw new NotImplementedException
                    ("GetOutlinedPathGeometry, Geometry.Combine is not implemented by avalonia.");
    ...
    private static void GenerateGeometry (StreamGeometryContext ctx, Rect rect, BorderInfo borderInfo)
    {
        ...
        if (leftTop.X > rightTop.X)
        {
            ...
            leftTop = new Point (v, leftTop.Y);
            rightTop = new Point (v, rightTop.Y);
        }
        ...
        if (topRight.Y > bottomRight.Y)
        {
            ...
            topRight = new Point (topRight.X, v);
            bottomRight = new Point (bottomRight.X, v);
        }
        ...
        if (leftBottom.X > rightBottom.X)
        {
            ...
            rightBottom = new Point (v, rightBottom.Y);
            leftBottom = new Point (v, leftBottom.Y);
        }
        ...
        if (topLeft.Y > bottomLeft.Y)
        {
            ...
            bottomLeft = new Point (bottomLeft.X, v);
            topLeft = new Point (topLeft.X, v);
        }
        ...
        if (!radiusX.IsZero() || !radiusY.IsZero())
        {
            ...
            ctx.ArcTo (topRight, new Size (radiusX, radiusY), 0, false, SweepDirection.Clockwise);
        }
        ...
        if (!radiusX.IsZero() || !radiusY.IsZero())
        {
            ...
            ctx.ArcTo (rightBottom, new Size (radiusX, radiusY), 0, false, SweepDirection.Clockwise);
        }
        ...
        if (!radiusX.IsZero() || !radiusY.IsZero())
        {
            ...
            ctx.ArcTo (bottomLeft, new Size (radiusX, radiusY), 0, false, SweepDirection.Clockwise);
        }
        ...
        if (!radiusX.IsZero() || !radiusY.IsZero())
        {
            ...
            ctx.ArcTo (leftTop, new Size (radiusX, radiusY), 0, false, SweepDirection.Clockwise);
        }
    }
