    ...
    : IDisposable
{
    ...
    /// <param name="quietZoneSize"></param>
    /// <returns></returns>
    ...
    private static class ModulePlacer
    {
        public static void AddQuietZone (ref QRCodeData qrCode, int quietZoneSize)
        {
            ...
            {
                var quietPart = new bool[quietZoneSize];
        ...
        public static void PlaceFormat (ref QRCodeData qrCode, string formatStr)
        {
            ...
            var modules = new[,]
            {
                { 8, 0, size - 1, 8 }, { 8, 1, size - 2, 8 }, { 8, 2, size - 3, 8 }, { 8, 3, size - 4, 8 },
                { 8, 4, size - 5, 8 }, { 8, 5, size - 6, 8 }, { 8, 7, size - 7, 8 }, { 8, 8, size - 8, 8 },
                { 7, 8, 8, size - 7 }, { 5, 8, 8, size - 6 }, { 4, 8, 8, size - 5 }, { 3, 8, 8, size - 4 },
                { 2, 8, 8, size - 3 }, { 1, 8, 8, size - 2 }, { 0, 8, 8, size - 1 }
            };
            for (var i = 0; i < 15; i++)
            {
        ...
        public static void PlaceFinderPatterns (ref QRCodeData qrCode, ref List<Rectangle> blockedModules)
        {
            ...
            for (var i = 0; i < 6; i = i + 2)
            {
                for (var x = 0; x < 7; x++)
                {
                    for (var y = 0; y < 7; y++)
                    {
                        if (!(((x == 1 || x == 5) && y is > 0 and < 6) || (x is > 0 and < 6 && (y == 1 || y == 5))))
                        {
    ...
    private string PlainTextToBinary (string plainText, EncodingMode encMode, EciMode eciMode, bool utf8BOM,
        bool forceUtf8)
    {
        switch (encMode)
        {
            ...
            switch (eciMode)
            {
                ...
                case EciMode.Iso8859_2:
                    codeBytes = Encoding.GetEncoding ("ISO-8859-2")
                        .GetBytes (ConvertToIso8859 (plainText, "ISO-8859-2"));
                    break;
    ...
    private Polynom MultiplyAlphaPolynoms (Polynom polynomBase, Polynom polynomMultiplier)
    {
        ...
        {
            var coefficient = resultPolynom.PolyItems.Where (x => x.Exponent == exponent).Aggregate (0,
                (current, polynomOld)
