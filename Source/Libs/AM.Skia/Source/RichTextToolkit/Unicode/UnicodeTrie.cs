...
internal class UnicodeTrie
{
    ...
    public uint Get (int codePoint)
    {
        ...
        if ((codePoint < 0xd800) || ((codePoint > 0xdbff) && (codePoint <= 0xffff)))
        {
            ...
            index = (_data[codePoint >> UnicodeTrieBuilder.SHIFT_2] << UnicodeTrieBuilder.INDEX_SHIFT) +
                    (codePoint & UnicodeTrieBuilder.DATA_MASK);
        ...
        if (codePoint <= 0xffff)
        {
            ...
            index =
                (_data[UnicodeTrieBuilder.LSCP_INDEX_2_OFFSET + ((codePoint - 0xd800) >> UnicodeTrieBuilder.SHIFT_2)] <<
                 UnicodeTrieBuilder.INDEX_SHIFT) + (codePoint & UnicodeTrieBuilder.DATA_MASK);
            return (uint)_data[index];
        }
        ...
        if (codePoint < _highStart)
        {
            ...
            index = _data[
                (UnicodeTrieBuilder.INDEX_1_OFFSET - UnicodeTrieBuilder.OMITTED_BMP_INDEX_1_LENGTH) +
                (codePoint >> UnicodeTrieBuilder.SHIFT_1)];
