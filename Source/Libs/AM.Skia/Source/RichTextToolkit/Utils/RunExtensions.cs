...
public static class RunExtensions
{
        ...
        where T : IRun
    {
        ...
        // Find the start run
        var startRunIndex = list.BinarySearch (offset, (r, a) =>
        {
            if (r.Offset > a)
            {
                return 1;
        ...
        for (var i = startRunIndex; i < list.Count; i++)
        {
            // Get the run
            var r = list[i];
    ...
    /// <param name="length">The length of the run</param>
    /// <returns>An enumerable collection of SubRuns</returns>
    public static IEnumerable<SubRun> GetIntersectingRunsReverse<T> (this IReadOnlyList<T> list, int offset, int length)
        where T : IRun
    {
        // Check list is consistent
        list.CheckValid();
        ...
        // Find the start run
        var endRunIndex = list.BinarySearch (to, (r, a) =>
        {
            if (r.Offset >= a)
            {
                return 1;
