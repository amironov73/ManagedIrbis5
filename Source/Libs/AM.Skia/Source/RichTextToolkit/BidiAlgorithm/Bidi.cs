...
internal class Bidi
{
    ...
    {
        Process (data.Types, data.PairedBracketTypes, data.PairedBracketValues, data.ParagraphEmbeddingLevel,
            data.HasBrackets, data.HasEmbeddings, data.HasIsolates, null);
    }
    ...
    /// </summary>
    private Slice<Directionality> _originalTypes;

    /// <summary>
    /// Paired bracket types as provided by caller
    /// </summary>
    private Slice<PairedBracketType> _pairedBracketTypes;
    ...
    /// </summary>
    private Slice<Directionality> _workingTypes;
    ...
    private void FindIsolatePairs()
    {
        ...
        for (var i = 0; i < _originalTypes.Length; i++)
        {
            var t = _originalTypes[i];
            if (t is Directionality.LRI or Directionality.RLI or Directionality.FSI)
            {
    ...
    private void AddLevelRun (int start, int length, int level)
    {
        ...
        int nextLevel;
        if (lastType is Directionality.LRI or Directionality.RLI or Directionality.FSI)
        {
    ...
    /// </summary>
    private Directionality _runDirection;
    ...
    /// being processed
    /// </summary>
    private MappedSlice<Directionality> _runResolvedTypes;

    /// <summary>
    /// A mapped slice of the original types for the isolating run currently
    /// being processed
    /// </summary>
    private MappedSlice<Directionality> _runOriginalTypes;
    ...
    /// </summary>
    private MappedSlice<PairedBracketType> _runPairedBracketTypes;
    ...
    /// </summary>
    private void ProcessIsolatedRunSequence (Directionality sos, Directionality eos, int runLevel)
    {
        ...
        if (_hasBrackets)
        {
            _runPairedBracketTypes =
                new MappedSlice<PairedBracketType> (_pairedBracketTypes, _isolatedRunMapping.AsSlice());
            ...
            for (i = 0; i < _runLength; i++)
            {
                if (_runResolvedTypes[i] == Directionality.EN)
                {
                    for (var j = i - 1; j >= 0; j--)
                    {
                        ...
                        if (t is Directionality.L or Directionality.R or Directionality.AL)
                        {
        ...
        if (hasES || hasET || hasCS)
        {
            for (i = 0; i < _runLength; ++i)
            {
                ...
                if (t is Directionality.ES or Directionality.ET or Directionality.CS)
                {
                    t = Directionality.ON;
                }
            }
        ...
        if (hasEN)
        {
            ...
            for (i = 0; i < _runLength; ++i)
            {
                ...
                if (rt is Directionality.L or Directionality.R)
                {
                    prevStrongType = rt;
                }
            }
        ...
        if (_hasBrackets)
        {
            ...
            for (i = 0, count = pairedBrackets.Count; i < count; i++)
            {
                ...
                if (dir is Directionality.L or Directionality.R && dir == _runDirection)
                {
        ...
        for (i = 0; i < _runLength; ++i)
        {
            ...
            if (IsNeutralType (t))
            {
                ...
                else
                {
                    ...
                    if (typeBefore is Directionality.AN or Directionality.EN)
                    {
                ...
                else
                {
                    ...
                    if (typeAfter is Directionality.AN or Directionality.EN)
                    {
        ...
        if ((_runLevel & 0x01) == 0)
        {
            ...
            for (i = 0; i < _runLength; i++)
            {
                ...
                else if (t is Directionality.AN or Directionality.EN)
                {
                    l += 2;
                }
            }
    ...
    /// </summary>
    private class PairedBracketComparer : IComparer<BracketPair>
    {
        ...
        for (int ich = 0, length = _runLength; ich < length; ich++)
        {
            ...
            switch (_runPairedBracketTypes[ich])
            {
                ...
                case PairedBracketType.c:
                    ...
                    for (var i = 0; i < _pendingOpeningBrackets.Count; i++)
                    {
    ...
    private Directionality InspectPairedBracket (BracketPair pb)
    {
    ...
    private Directionality InspectBeforePairedBracket (BracketPair pb, Directionality sos)
    {
    ...
    private void SetPairedBracketDirection (BracketPair pb, Directionality dir)
    {
        ...
        for (var i = pb.ClosingIndex + 1; i < _runLength; i++)
        {
            if (_runOriginalTypes[i] == Directionality.NSM)
            {
    ...
    private void ResetWhitespaceLevels()
    {
        ...
        {
            var t = _originalTypes[i];
            if (t is Directionality.B or Directionality.S)
            {
    ...
    /// <returns></returns>
    [MethodImpl (MethodImplOptions.AggressiveInlining)]
    private bool IsNeutralType (Directionality dir)
    {
        switch (dir)
        {
            case Directionality.B:
    ...
    [MethodImpl (MethodImplOptions.AggressiveInlining)]
    private Directionality GetStrongTypeN0 (Directionality dir)
    {
        switch (dir)
        {
            case Directionality.EN:
