...
public class TextLine
{
    ...
    /// </remarks>
    public float MaxAscent { get; internal set; }
    ...
    /// </remarks>
    public float MaxDescent { get; internal set; }
    ...
    internal void HitTest (float x, ref HitTestResult htr)
    {
        ...
        if (Runs.Count > 0)
        {
                ...
                (lastRun.Direction == TextDirection.RTL && x < lastRun.XCoord))
            {
                ...
                if (lastRun.RunKind is FontRunKind.TrailingWhitespace or FontRunKind.Ellipsis)
                {
        ...
        foreach (var r in Runs)
        {
            ...
            {
                // Inside the run
                for (var i = 0; i < r.Clusters.Length;)
                {
                    ...
                    if (x >= xcoord1 && x < xcoord2)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                htr.ClosestCodePointIndex = r.Direction == TextDirection.LTR
                                    ? codePointIndex
                                    : codePointIndexOther;
                            }
                            else
