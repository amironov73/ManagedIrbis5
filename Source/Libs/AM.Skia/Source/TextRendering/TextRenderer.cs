...
public static class TextRenderer
{
    ...
    private static int GetFirstWordLength (string textLine, int startIndex)
    {
        ...
        for (var index = startIndex; index < textLine.Length; ++index)
        {
            var character = textLine[index];

            if (character == UnicodeCharacters.NewLine || character == UnicodeCharacters.CarriageReturn
                                                       || (!Utils.IsUnicodeNonBreaking (character) &&
                                                           (Utils.IsUnicodeSpace (character) ||
                                                            Utils.IsCJKUnicode (character))))
            {
                break;
            }
    ...
    private static void MultilineTextWrap (GetFirstCharOrWordLength nextTokenLength)
    {
        ...
        for (var index = 0; index < textLength;)
        {
            ...
            var whitespaceWidth = nextWhitespaceWidth;
            var newLine = false;
            for (var tmp = 0; tmp < tokenLen; ++tmp)
            {
                ...
                if (!FontCache.GetLetterDefinitionForChar (character, out letterDef))
                {
                    RecordPlaceholderInfo (letterIndex, character);
                    Console.WriteLine (
                        $"TextRenderer.MultilineTextWrap error: can't find letter definition in font file for letter: {character}");
                    continue;
                }
    ...
    private static bool DrawSelectionIfNeed (SKCanvas canvas)
    {
        ...
        startLetter = null;
        for (var i = PaintOptions.SelectionStart.Value; i <= PaintOptions.SelectionEnd.Value; i++)
        {
            ...
            if ((currentLetter.Character == UnicodeCharacters.NewLine ||
                 currentLetter.Character == UnicodeCharacters.CarriageReturn) ||
        ...
        // Draw underline/strikethrough and texts
        {
            ...
            for (var i = 0; i < Text.Length; i++)
            {
                ...
                if (TextStyle == FontStyle.Underline || TextStyle == FontStyle.Strikeout)
                {
                    ...
                    realPosY += TextStyle == FontStyle.Underline
                        ? (TextPaint.FontMetrics.UnderlinePosition ?? 0)
                        : (TextPaint.FontMetrics.StrikeoutPosition ?? 0);
                    canvas.DrawLine (new SKPoint (letterInfo.PositionX, realPosY),
                        new SKPoint (letterInfo.PositionX + LinesWidth[letterInfo.LineIndex], realPosY), TextPaint);
    ...
    public static void DrawText (SKCanvas canvas, string text, Font font, SKRect bounds, SKColor foreColor,
