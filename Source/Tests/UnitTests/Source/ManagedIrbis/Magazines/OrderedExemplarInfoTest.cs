...
[TestClass]
public sealed class OrderedExemplarInfoTest
{
    private Field _GetField()
    {
        return new Field
            (
                OrderedExemplarInfo.Tag,
                "^q2022/0^a0^b1^dФ304^fЕ2^e339.59^kФ304 - экз.1"
            );
    }

    private OrderedExemplarInfo _GetOrderInfo()
    {
        return new OrderedExemplarInfo
        {
            ...
            Channel = "Е2",
            Price = "339.59",
            UnknownSubfields = new SubField [] { new ('k', "Ф304 - экз.1") }
        };
    }
    ...
    private void _Compare
        (
            OrderedExemplarInfo first,
            OrderedExemplarInfo second
        )
    {
        Assert.AreEqual (first.Year, second.Year);
        Assert.AreEqual (first.Status, second.Status);
        Assert.AreEqual (first.Complect, second.Complect);
        Assert.AreEqual (first.StartingNumber, second.StartingNumber);
        Assert.AreEqual (first.Place, second.Place);
        Assert.AreEqual (first.StorageDuration, second.StorageDuration);
        ...
        Assert.AreEqual (first.Recalc, second.Recalc);
        Assert.AreEqual (first.FirstPoint, second.FirstPoint);
        Assert.AreEqual (first.SecondPoint, second.SecondPoint);
        Assert.AreEqual (first.ThirdPoint, second.ThirdPoint);
        Assert.AreEqual (first.FourthPoint, second.FourthPoint);
    ...
    public void OrderedExemplarInfo_Construction_1()
    {
        var order = new OrderedExemplarInfo();
        Assert.IsNull (order.Year);
        Assert.IsNull (order.Status);
        Assert.IsNull (order.Complect);
        Assert.IsNull (order.StartingNumber);
        Assert.IsNull (order.Place);
        Assert.IsNull (order.StorageDuration);
        Assert.IsNull (order.Amount);
        Assert.IsNull (order.Channel);
        Assert.IsNull (order.Price);
        Assert.IsNull (order.Recalc);
        Assert.IsNull (order.FirstPoint);
        Assert.IsNull (order.SecondPoint);
        Assert.IsNull (order.ThirdPoint);
        Assert.IsNull (order.FourthPoint);
        Assert.IsNull (order.FifthPoint);
    ...
    public void OrderedExemplarInfo_ParseField_1()
    {
        var field = _GetField();

        var actual = OrderedExemplarInfo.ParseField (field);
        _Compare (_GetOrderInfo(), actual);
    }

    [TestMethod]
    [Description ("Разбор указанной библиографической записи")]
    public void OrderedExemplarInfo_ParseRecord_1()
    {
        var record = _GetRecord();
        var orders = OrderedExemplarInfo.ParseRecord (record);
        Assert.AreEqual (16, orders.Length);
        var year2017 = orders.FirstOrDefault (o => o.Year!.Contains ("2017"));
        Assert.IsNotNull (year2017);
    }

    [TestMethod]
    [Description ("Преобразование информации в поле библиографической записи")]
    public void OrderedExemplarInfo_ToField_1()
    {
        var expected = _GetField();
        var order = _GetOrderInfo();
        var actual = order.ToField();
        foreach (var code in OrderedExemplarInfo.KnownCodes)
        {
    ...
    public void OrderedExemplarInfo_ApplyToField_1()
    {
        var expected = _GetField();
        var actual = new Field (OrderedExemplarInfo.Tag)
            .Add ('a', "555")
            .Add ('b', "666");
        var order = _GetOrderInfo();
        ...
        )
    {
        ...
        var second = bytes.RestoreObjectFromMemory<OrderedExemplarInfo>();
    ...
    public void OrderedExemplarInfo_Serialization_1()
    {
        var order = new OrderedExemplarInfo();
    ...
    public void OrderedExemplarInfo_Verify_1()
    {
        var order = new OrderedExemplarInfo();
    ...
    public void OrderedExemplarInfo_ToXml_1()
    {
        var order = new OrderedExemplarInfo();
        Assert.AreEqual ("<ordered />", XmlUtility.SerializeShort (order));

        order = _GetOrderInfo();
        Assert.AreEqual ("<ordered year=\"2022/0\" status=\"0\" complect=\"1\" place=\"Ф304\" channel=\"Е2\" price=\"339.59\"><unknown code=\"k\" value=\"Ф304 - экз.1\" /></ordered>",
            XmlUtility.SerializeShort (order));
    }

    [TestMethod]
    [Description ("JSON-представление")]
    public void OrderedExemplarInfo_ToJson_1()
    {
        var order = new OrderedExemplarInfo();
        Assert.AreEqual ("{}", JsonUtility.SerializeShort (order));

        order = _GetOrderInfo();
        Assert.AreEqual ("{\"year\":\"2022/0\",\"status\":\"0\",\"complect\":\"1\",\"place\":\"\\u0424304\",\"channel\":\"\\u04152\",\"price\":\"339.59\",\"unknown\":[{\"Code\":\"k\",\"Value\":\"\\u0424304 - \\u044D\\u043A\\u0437.1\",\"RepresentsValue\":false}]}",
            JsonUtility.SerializeShort (order));
    }

    [TestMethod]
    [Description ("Плоское текстовое представление")]
    public void OrderedExemplarInfo_ToString_1()
    {
        var order = new OrderedExemplarInfo();
        Assert.AreEqual ("(null) (null) (null)", order.ToString());

        order = _GetOrderInfo();
        Assert.AreEqual ("2022/0 1 0", order.ToString());
    }
}
