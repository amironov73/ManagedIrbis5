...
namespace UnitTests.ManagedIrbis.Quality
{
    ...
    public class RecordReportTest
    {
        ...
        {
            var report = new RecordReport();
            Assert.AreEqual("{\"mfn\":0,\"defects\":[],\"quality\":0}", JsonUtility.SerializeShort(report));

            report.Mfn = 1;
            report.Quality = 100;
            report.Description = "Description";
            report.Index = "Index";
            report.Defects.Add(new FieldDefect { Damage = 100, Field = 200, Message = "Message" });
            Assert.AreEqual
                (
                    "{\"mfn\":1,\"index\":\"Index\",\"description\":\"Description\",\"defects\":[{\"field\":200,\"message\":\"Message\",\"damage\":100}],\"quality\":100}",
                    JsonUtility.SerializeShort(report)
                );
        }
