...
public sealed class NumberRangeTest
{
    ...
    [TestMethod]
    [Description ("Номера через косую черту")]
    public void NumberRange_Parse_13()
    {
        var actual = NumberRange.Parse ("2/3");
        Assert.AreEqual (new NumberText("2"), actual.Start);
        Assert.AreEqual (new NumberText("3"), actual.Stop);
    }
