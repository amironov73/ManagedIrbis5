...
* **AtLeastOnceUntil<U> (Parser<TToken, U> terminator)** -- повторно выполняет первый парсер до тех пор, пока не сработает второй, результирующая последовательность должна содержать не менее одного элемента;
...
* **Separated<U> (Parser<TToken, U> separator)** -- возвращает список `IEnumerable<T>`, значения типа `T` перемежаются значениями типа `U` (проще говоря список через запятую), возвращает от нуля до бесконечности значений;
* **SeparatedAtLeastOnce<U> (Parser<TToken, U> separator)** -- аналог предыдущего случая, однако, возвращает не менее одного элемента в списке;
* **SeparatedAndTerminated<U> (Parser<TToken, U> separator)** -- аналог предыдущего случая, однако, список может быть пустым, но должен также завершаться значением типа `U`;
* **SeparatedAndTerminatedAtLestOnce<U> (Parser<TToken, U> separator)** -- аналог предыдущего случая, однако, возвращает не менее одного элемента;
* **SeparatedAndOptionallyTerminated<U> (Parser<TToken, U> separator)** -- аналог предыдущего случая, однако, список может быть пустым и опционально завершается указанным значением;
* **SeparatedAndOptionallyTerminatedAtLeastOnce<U> (Parser<TToken, U> separator)** -- очередная вариация;
* **SkipAtLeastOnceUntil<U> (Parser<TToken, U> terminator)** -- повторно выполняет первый парсер (не менее одного раза!) (отбрасывая результаты), пока не сработает второй;
* **SkipUntil<U> (Parser<TToken, U> terminator)** -- повторно выполняет первый парсер (отбрасывая его результаты), пока не сработает второй;
* **Then** -- при успехе обоих парсеров выдает результат второго;
* **Until<U> (Parser<TToken, U> terminator)** -- повторно выполняет первый парсер до тех пор, пока не сработает второй;
...
* **Bind<U> (Func<T, Parser<TToken, U>> selector)** -- применяет трансформирующую функцию к результату парсера;
* **Cast<U>()** -- кастует результат к указанному типу;
...
* **OfType<U>()** -- кастует результат к указанному типу. См `Cast`;
...
* **Select<U> (Func<T, U> selector)** -- синоним `Map` для случая одного парсера;
* **ThenReturn<U> (U result)** -- заменяет результат для текущего парсера;
* **Try (Parser<TToken, T> parser)** -- если заданный парсер потерпел неудачу, выполняет "откат";
* **WithResult<U> (U result)** -- заменяет результат для текущего парсера, синоним для `ThenReturn`;
