...
internal class Record
{
  ...
  public Field FindField (string tag) =>
      this.Fields.Where<Field>((Func<Field, bool>) (field => field.Tag == tag))
      ...
      )
  {
    ...
    {
      string tag = tag1;
      foreach (Field field in this.Fields.Where<Field> ((Func<Field, bool>)(field => field.Tag == tag)).ToArray<Field>())
      {
      ...
      )
  {
    ...
    return !string.IsNullOrEmpty(str)
        ? str
        : throw new ApplicationException ("End of stream encountered");
  }
  ...
  {
    using (var reader = new StreamReader (fileName, Record.UsedEncoding))
    {
  ...
  {
      using (var streamWriter = new StreamWriter (fileName, false, Record.UsedEncoding))
      {
