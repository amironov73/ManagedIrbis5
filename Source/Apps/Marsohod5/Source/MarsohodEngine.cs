    ...
    : BackgroundService
{
    ...
    private void LoadMagazines()
    {
        ...
        _logger.LogInformation ("Records found: {Count}", records.Length);
        ...
        _logger.LogInformation ("Magazines loaded: {Count}", _magazines.Count);
    }
        ...
        )
    {
        ...
        _logger.LogInformation ("File found: {FileName}", inputFile);
        var match = _nameRegex.Match (fileName);
        if (!match.Success)
        {
            _logger.LogInformation ("Wrong file name pattern: {FileName}", fileName);
            return;
        }
        ...
        _logger.LogInformation
            (
                "File {FileName} contains {Count} records",
                fileName,
                counter
            );
    }
        ...
        )
    {
        while (!channel.Completion.IsCompleted)
        {
            ...
            _logger.LogInformation ("Imported issue {Issue}", value.CurrentIssue);
        }
    }
