...
* ``Link`` - ссылка на ресурс (например, на электронный экземпляр книги).

    * ``url`` - собственно ссылка.
    * ``description`` - описание в свободной форме, например, ``"Книга в ЭБС"``.
...
* ``Exemplar`` - экземпляр книги (или другого документа) из фондов библиотеки.
    ...
    * ``status`` - строка: статус данного экземпляра. Статус ``"ok"`` означает доступность экземпляра для выдачи, остальные значения - недоступность по тем или иным причинам (например, книга находится на руках у читателя).
...
* ``Book`` - книга (или другой документ) из фондов библиотеки.
    ...
    * ``arrangement`` - строка с HTML-расстановочным шифром книги, например, ``"84(2)<br>А 12"``.
    ...
    * ``links`` - массив объектов ``Link`` (см. выше) - ссылки, сопровождающие книгу, может быть ``null``.
    ...
    * ``cards`` - массив строк с ссылками на изображения сканированных карточек, может быть ``null``.

* ``Order`` - описание заказа.

    * ``id`` число: идентификатор заказа, например, ``123``.
    * ``ticket`` - строка: номер билета читателя, сделавшего заказ, например, ``"A001"``.
    * ``description`` - строка: библиографическое описание заказанной книги, например, ``"Сказки Пушкина"``.
    * ``instance`` - заказанный экземпляр (см. выше ``Instance``).
    * ``date`` - дата размещения заказа.
